...
{
  "name": "ui-design",
  "description": "Apply a strong product-design critique and implementation workflow for polished web interfaces and visual systems.",
  "version": "1.0.0",
  "category": "design",
  "platforms": [
    "linux",
  ...
  "playbook": [
    {
      "id": "inspect-product-context",
      "description": "Understand the app purpose, audience, current design system, and constraints.",
      "toolsets": [
        "files",
        "browser"
      ]
    ...
    {
      "id": "make-focused-design-change",
      "description": "Implement scoped UI improvements using the existing stack and design patterns.",
      "toolsets": [
        "files",
        "coding"
      ]
    ...
    {
      "id": "verify-visual-result",
      "description": "Use Browser screenshots or inspection where possible to catch layout regressions.",
      "toolsets": [
        "browser"
      ]
    }
  ...
  "evaluations": [
    {
      "input": "Polish this dashboard so it feels like a mature SaaS app.",
      "shouldUseToolsets": [
        "files",
        "browser"
      ]
...
# UI Design for CLI/API Agents

Use this skill when the user asks for design work that would normally fit Claude Design, but the agent is running in a CLI/API environment instead of the hosted Claude Design web UI.

Preserve the design taste and process; remove hosted-tool plumbing.

**Before starting, decide whether the task is a rendered UI/design implementation or a durable spec.** Use this for interfaces and polish; use `product-design` for persistent token/spec files.

## When To Use This Skill vs `product-design`

Choose the right design skill, or combine them:

| Skill | What it gives you | Use when the user wants... |
|---|---|---|
| **ui-design** | Design process and taste: scope, context, variants, local HTML verification, anti-slop discipline | from-scratch artifacts: landing pages, prototypes, decks, component labs, motion studies |
| **product-design** | DESIGN.md specs: tokens, rationale, validation, contrast checks, Tailwind/DTCG export | persistent machine-readable design-system spec files |

Rule of thumb:

- **Process + taste, one-off artifact** → ui-design
- **Match a known brand's look** → use provided references, screenshots, or web research; let this skill drive the process.
- **Author the tokens spec itself** → product-design

Compose them: this skill for rendered experience; `product-design` for token/spec files.

## Runtime Mode

You are running in **CLI/API mode**, not the Claude Design hosted web UI.

Ignore hosted-only tools, project/preview panes, toolbar protocols, callbacks, and platform paths unavailable here.
...
Do not use this skill for pure DESIGN.md token authoring unless the user specifically asks for a DESIGN.md file. Use `product-design` for that.
...
The hosted Claude Design edit toolbar does not exist here. Preserve the idea with in-page `Tweaks` controls when useful.
...
```text
CLI/API mode, not hosted Claude Design. Ignore hosted-only tools/panes. Produce complete local artifacts, usually self-contained HTML with CSS/JS. Verify locally. Preserve the process: gather context, define the system, produce options, avoid filler, and meet a high visual bar.
```
