---
{
  ...
  "playbook": [
    ...
    },
    {
      ...
      "description": "Read back the destination's imported API resource and protected-value metadata using configured verification tools. Inspect the effective request URL, variable scope precedence, and authentication/body variable references, not just resource existence. Preserve product-specific base paths; shared variables must not override them incorrectly. Wire grounded credential roles through variable references, never literal secrets or unresolved import placeholders. Use existing metadata and protected receipts without requesting values again. Distinguish resources present, configuration verified, and live authentication tested; sending a request requires user authorization for that action. Continue independent products when one remains unresolved.",
      "toolsets": ["mcp"]
    }
  ],
  "evaluations": [
...
Configuration verification is more than an existence check. Resolve the intended request URL using the destination's actual variable precedence; do not assume collection values override environment values. Keep product-specific API paths intact and avoid a shared base URL that silently shadows them. Inspect authentication and body fields for unresolved generated placeholders, and connect their grounded roles to the intended credential variable references. Storing a protected value does not automatically connect it to a request. Preserve unrelated settings and credentials when correcting configuration. This is verification guidance, not a new runtime gate or permission to send test requests. Report configuration readiness separately from live authentication or API testing.

Prefer a field-level or request-specific update for a small correction. Before replacing an entire resource, confirm that the write schema can express its existing nested fields, including request bodies and saved examples. A successful write response does not prove those fields survived; verify both the intended change and preservation of unrelated content. If a connector cannot represent the complete resource, use an available narrower update instead of sending a lossy replacement.
