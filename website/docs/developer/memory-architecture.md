...
Only one finalization job may run per profile. Expired queue and curation leases are recoverable; failures retry with bounded backoff before becoming terminal. Checkpoints, `memory.curate`, automatic promotions, and `MemoryOperatorCommands` writes use the same profile lease rather than racing a finalizer. The worker retains the latest 1,000 terminal rows per profile. `memory status` and `gateway status` aggregate counts without transcript data; top-level `memory finalization list`, `retry`, and `prune` provide profile-scoped recovery.
