...
`SQLiteSessionDB` installs the profile-owned Task schema and `SQLiteTaskStore` provides transactional graph storage. SQLite runtimes also create a profile-local `TaskResultService`: result bodies remain outside SQLite, carry verified hashes and opaque handles, and are available to linked sessions through bounded `task.result.read` pages. Binary results are never injected through that text tool. A deterministic scheduler and production `AgentStepExecutor` are tested together: the executor rechecks profile, workspace binding and trust, narrows the child to read-only capabilities, checkpoints worker ownership under the Attempt fence, captures complete results, and accounts for every provider attempt. `createRuntime()` still does not construct a scheduler host, so this path remains unavailable to ordinary users until supervisor and product-surface wiring lands. Retired Workflow commands fail explicitly; runtime creation does not silently initialize a legacy store.
