...
`SQLiteSessionDB` installs the profile-owned Task schema and `SQLiteTaskStore` provides transactional graph storage. SQLite runtimes also create a profile-local `TaskResultService`: result bodies remain outside SQLite, carry verified hashes and opaque handles, and are available to linked sessions through bounded `task.result.read` pages. Binary results are never injected through that text tool. A deterministic scheduler core exists for isolated fake-executor and restart testing, but `createRuntime()` does not construct it and no production Step executor is registered. Retired Workflow commands fail explicitly; runtime creation does not silently initialize a legacy store.
