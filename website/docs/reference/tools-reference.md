...
For cross-system work, `write` can include bounded semantic `requirements` tied to plan items and exact session-visible tool names. The runtime derives risk, reviewed protected paths, grouped delivery, browser relay, and independent verification relationships from registered tools, then checks them against the final filtered session registry. Only the runtime writes resolved readiness assessments. Missing or incompatible mandatory capabilities block the Mission before substantive source-side work; this inspection invokes no destination tool, grants no approval, and does not claim remote account permissions.

Completed action items must cite successful tool call IDs. EstaCoda resolves those IDs through a session-local evidence index and stores only derived, redacted receipts; failed, blocked, unavailable, planning, and delegation calls are rejected as proof. Genuinely reasoning-only items may declare `completionKind: "reasoning"`. A merge cannot complete the final unfinished existing objective item and append new unfinished work in the same transition. Required discoveries can still be added while original work remains; optional post-success cleanup is reported as follow-up rather than silently extending the Mission. An unfinished Mission receives one automatic recovery continuation before EstaCoda returns a bounded incomplete receipt. When the Mission is blocked on required user input, a non-empty response or explicit retry reopens the first such item without granting tool authority; other blocker kinds remain blocked. Terminal Missions leave the live CLI panel while retaining their durable plain-text summary where applicable.
...
Synthesis is fail-closed: it receives bounded accepted-Result references with exact `task.result.read` inputs, cannot delegate, and runs only after all workers complete. A worker failure skips synthesis and leaves the Task `partial`. Safe output recovered from a failed Attempt is labeled **Recovered output** and remains authorized for inspection, but it never satisfies dependencies or becomes a successful or primary Result. Approval-denied, policy-blocked, and mutating-tool failure output is not automatically recovered. A successful synthesis Result is marked primary on status surfaces and completion delivery; intermediate accepted Results remain available by opaque handle.

When the returned metadata identifies a non-terminal root Task with a `primaryResultStepId`, that Task owns the requested answer. EstaCoda stops the parent provider loop after creation and returns deterministic “Task created” text with the Task ID and current execution state. The provider cannot generate a substitute answer while workers remain outstanding. The accepted primary Result is delivered through the existing CLI or channel outbox when the Task settles; a Task without an accepted answer delivers a deterministic failure receipt. Nested child Tasks remain internal, and `synthesis: false` is inspection-only and retains ordinary continuation behavior.

The default Step policy keeps read-only local/network tools from the final provider-visible inventory and applies exact blocks, prefix blocks, and excluded toolsets. Explicit `allowedTools` and `allowedToolsets` are required constraints: if a requested capability is unavailable, or the intersection is empty, no Task is created and the tool returns structured diagnostics. Successful Steps persist a bounded access audit, and worker construction rejects zero or widened access before contacting the provider. Worker Steps cannot delegate. Orchestrator Steps may retain `delegate_task` only with persisted child authority and remaining depth. A linked child Task must keep the parent workspace and receive narrower authority and budget.

Each research item may add a `research` contract with a bounded scope plus `requireLiveSources` and `requireRepositoryEvidence`. Batch scopes must be distinct after normalization. Live-source work requires effective `web.search`; repository evidence requires `file.read` and one of `file.search`, `file.grep`, or `file.glob`, otherwise Task creation fails. A successful Result must cite URLs and workspace-relative paths observed in successful tool results. Training-only or fabricated evidence fails as `evidence-contract-unsatisfied`; safe text is diagnostic-only and is not available to synthesis. Accepted synthesis references retain their scope, and unavailable scopes remain explicit in partial coverage.
