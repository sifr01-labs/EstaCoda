...
**Protected forms:** `browser.fill_protected_form` groups one to eight current refs from the same revision, tab, frame, and origin. EstaCoda verifies all fields before collection and again before delivery, shows one continuous operator input flow, never returns the protected values to the model, and never submits the form. Account identifiers, passwords, and later one-time-code challenges are separate typed requests. A one-time-code `browser.type` request may include a same-revision `submitRef`; the local runtime binds both controls before prompting, enters and submits the code without another model turn, then returns a sanitized settlement receipt. Submission is not reported as proof of authentication.
