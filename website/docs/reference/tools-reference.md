...
For cross-system work, `write` can include bounded `requirements` tied to plan items and exact session-visible tool names. The runtime checks read, mutation, independent verification, reviewed protected paths, and protected browser transfer against the final filtered session registry. Only the runtime writes readiness assessments. Missing or incompatible mandatory capabilities block the Mission before substantive source-side work; this inspection invokes no destination tool, grants no approval, and does not claim remote account permissions.
