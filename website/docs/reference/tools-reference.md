...
Synthesis is fail-closed: it receives bounded Result handles and `task.result.read` authority, cannot delegate, and runs only after all workers complete. A worker failure skips synthesis and leaves the Task `partial`. A successful synthesis Result is marked primary on status surfaces and completion delivery; intermediate Results remain available by opaque handle.
