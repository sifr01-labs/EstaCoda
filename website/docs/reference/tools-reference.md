...
**Behavior:** Atomically creates a fixed durable Task graph and returns its handle immediately. One task becomes one Step; a batch becomes independent Steps under one Task. Optional `synthesis.objective` adds one terminal synthesis Step whose revision-1 dependencies are every worker. Provider tool-call identity prevents duplicate creation. No synchronous child execution or in-memory persistence fallback remains.
