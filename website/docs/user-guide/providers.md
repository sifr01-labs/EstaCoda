...
Maintainers can run the opt-in scored release lane from a source checkout with `pnpm run eval:vision:live`; any route chain with a possible hosted destination requires `--consent-hosted` and has a `$1.00` maximum estimated exposure by default. Use `--max-cost-usd <amount>` to set another explicit run cap. JSON and Markdown reports cover OCR error rates, grounded facts, hallucinations, latency, cost, payload size, fallback observation, and approval frequency against stored thresholds. Ordinary provider variation inside the defined tolerance does not fail the gate.
