...
يمكن تشغيل مهمة إنهاء واحدة فقط لكل ملف شخصي. يمكن استعادة leases المنتهية للطابور والتنظيم؛ وتُعاد محاولة الإخفاقات بتأخير محدود قبل انتقالها إلى حالة نهائية. تستخدم نقاط التنظيم و`memory.curate` والترقيات التلقائية وكتابات `MemoryOperatorCommands` lease الملف الشخصي نفسها بدل التسابق مع المنظم. يحتفظ العامل بأحدث 1,000 صف نهائي لكل ملف شخصي. يجمع `memory status` و`gateway status` الأعداد دون بيانات النص، وتوفر أوامر `memory finalization list` و`retry` و`prune` استردادًا محليًا محددًا بالملف الشخصي.
