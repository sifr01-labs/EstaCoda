...
إلغاء الجولة بواسطة الإشارة الأصلية ومهلة المزوّد نتيجتان منفصلتان. إذا ألغت `AbortSignal` الأصلية تدفقًا نشطًا، تستخدم تشخيصات التدفق `finish: "cancelled"` ولا تعيّن `errorClass: "timeout"`؛ ويحمل حدث الجلسة `agent-cancelled` المقابل قيمة `abortSource` الأصلية المحدودة. تستمر مهلات المزوّد الكلية أو مهلات الخمول الحقيقية في استخدام `errorClass: "timeout"` مع نهاية تدفق من نوع خطأ. كما يمنع إلغاء الجولة الأصلية الإبلاغ عن الرجوع الاحتياطي لأن الجولة لم تعد مخولة بإرسال مسار آخر.
