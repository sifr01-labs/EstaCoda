...
للمسارات غير المدعومة، تستخدم المتابعة مسار النص المسطح `Executed tool results`. للمسارات الأصلية المدعومة، تُدرج مجموعات الأدوات المختارة كسجل `assistant`/`tool` منظم. يملك السجل الأصلي حدًا ثابتًا يقارب 12 ألف token بصرف النظر عن حجم سياق النموذج، وتبقى مجموعات `assistant`/`tool` ذرية. تُعاد تعبئة الوحدات الأصلية الأقدم غير المختارة في ملخصات محدودة. تبقى تعليمة المتابعة النهائية آخر رسالة `user`.

تبقى أحدث دفعة أدوات متاحة ضمن الحدود الحالية لكل نتيجة أداة. تتحول الدفعات الأقدم إلى إيصالات موجزة ومنقحة، ويُحدّ إجمالي ملاحظات المتابعة المسطحة بحوالي 12 ألف حرف. لا تُرسل الصور مرة أخرى إلا من أحدث دفعة، بينما تبقى مراجع artifacts متاحة بصورة مستقلة. إذا كانت رسالة `tool` أصلية مختارة تحمل بالفعل نتيجة من أحدث دفعة، فلا تتكرر النتيجة نفسها في كتلة المتابعة المسطحة.
