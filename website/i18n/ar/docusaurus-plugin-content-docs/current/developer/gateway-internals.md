...
| السلوك | الافتراضي |
|---|---|
| عتبة عدم النشاط | 5 دقائق دون حدث تقدم من بيئة التشغيل |
...
تحدّث أنشطة المزوّد وبدايات الأدوات ونتائجها وتقدم التفويض وغيرها من محطات بيئة التشغيل المحدودة الطابع الزمني لآخر تقدم في الجولة النشطة. تقارن فحوصات التعليق الوقت الحالي بهذا الطابع الزمني، ولا تزيد عداد فحوصات التعليق إلا بعد خمس دقائق بلا تقدم. يستطيع منطق المشرف تسجيل أحداث التعليق، وإجهاض الجولات غير النشطة، وتعليق بيئة تشغيل بعد دلائل متكررة ومميزة على حلقة عالقة للجلسة نفسها. عامل التعليق كمسار سلامة ومرونة، لا كضمان أن كل عملية مزوّد/أداة محجوبة يمكن التعافي منها بنظافة.

عند إلغاء الجولة بواسطة إشارتها الأصلية، يحتفظ حدث `agent-cancelled` الدائم بقيمة `reason` الحالية التي تصف مرحلة التنفيذ، ويسجل أيضًا `abortSource` محدودًا. تُطبّع مصادر البوابة والمشرف إلى `interrupt` أو `stop` أو `drain-timeout` أو `stuck-loop`؛ أما أسباب الإلغاء الأخرى فتُسجل كـ `unknown` بدل حفظ حمولات إشارة اعتباطية.
