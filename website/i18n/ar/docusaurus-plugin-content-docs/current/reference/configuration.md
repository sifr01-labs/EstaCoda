...
| `channels.telegram.streaming.editIntervalMs` | `750` | فاصل تجميع تعديلات Telegram الجزئية. |
| `channels.telegram.streaming.minInitialChars` | `24` | حد الأحرف المرئية بعد التصفية قبل أول إرسال جزئي. |
| `channels.telegram.streaming.cursor` | `"▌"` | مؤشر بث مؤقت يُلحق بالرسائل الجزئية. |
| `channels.telegram.streaming.maxFloodStrikes` | `2` | حد تدهور flood-control للدور النشط. |
| `channels.telegram.streaming.cleanupFailedAttempts` | `true` | حذف أو تحييد الرسائل المبثوثة المؤقتة بعد فشل المزود أو fallback. |

`DeliveryRouter` يعطل بث Telegram في v1. يستخدم البث الجزئي HTML escaping خفيفًا؛ أما التوصيل النهائي فيستخدم تنسيق Telegram العادي. flood control، أو الحمولات الجزئية الكبيرة، أو حدود الموافقة، أو حدود المنتجات، أو فشل final edit تفرض fallback للنص النهائي العادي للدور النشط فقط.
