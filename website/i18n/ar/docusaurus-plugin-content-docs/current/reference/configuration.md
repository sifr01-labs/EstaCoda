...
estacoda setup
→ الميزانيات
→ حد الإنفاق الافتراضي للمهام أو حد الإنفاق الافتراضي للجلسات
```

مدخل «الميزانيات» مخصص للتنقل فقط. تتم مراجعة حد المهمة وحد الجلسة وتطبيق كل منهما بصورة مستقلة. بعد التطبيق، يعود محرّر الإعدادات إلى قائمة الميزانيات الفرعية بالقيم المحدّثة؛ ولا يكتب «رجوع» أو إلغاء المراجعة أي إعدادات.

يؤدي حذف `budgets`، أو حذف النطاقين، إلى إبقاء الحدين متوقفين:
...
يمكن ضبط أحد النطاقين أو كليهما:
...
|---------|-----------|--------|
| `budgets.task` | محذوف / متوقف | حد افتراضي يُنسخ إلى كل مهمة جذرية جديدة ويشمل شجرة المهمة كاملة. |
| `budgets.session` | محذوف / متوقف | حد افتراضي يُنسخ إلى كل جلسة منطقية جديدة ويشمل نشاط المزوّد المنسوب إلى نطاق تلك الجلسة. |
| `maxEstimatedCostUsd` | مطلوب عند وجود النطاق | سقف محدود وغير سالب بالدولار الأمريكي. القيمة `0` حد صفري مفعّل للإنفاق وليست «متوقف». |
| `warningThresholdPercent` | `80` | عتبة تحذير من `0` إلى `100`، محسوبة من الإنفاق المسجّل والسعة المحجوزة حاليًا. |

تُطبّق هذه القيم الافتراضية على الأعمال الجديدة. لا يؤدي تغيير إعداد الملف الشخصي إلى إعادة كتابة سياسة الإنفاق الثابتة المحفوظة مسبقًا للمهام أو الجلسات الحالية.

#### النطاق والنشاط المشمول

يشمل حد المهمة العمال، والإخفاقات المدفوعة، وإعادات المحاولة، والمسارات الاحتياطية، والتوليف، والنماذج المساعدة، والمهام التابعة في شجرة المهمة الجذرية. ولا يمكن لمهمة تابعة إنشاء نطاق مالي منفصل أكبر.

يشمل حد الجلسة أدوار المحادثة الرئيسية، واستدعاءات النماذج المساعدة، والمهام الناشئة من الجلسة المنطقية. تحتفظ جلسات العمال بنطاق إنفاق الجلسة الأصلية. وعندما ينطبق الحدان، يجب أن يتسع كل من حد المهمة وحد الجلسة لطلب المزوّد.

تبقى أعداد الرموز وأعداد استدعاءات المزوّد تفاصيل استخدام وحوكمة هندسية، وليست إعدادات ميزانية موجهة للمستخدم.

#### التكلفة التقديرية والتنفيذ

تقدّر EstaCoda مقدار التعرض باستخدام لقطة موثوقة لتسعير النموذج، والمدخل المحدود، والحد الأقصى للخرج والاستدلال، وأسعار الذاكرة المؤقتة المنطبقة. قبل استدعاء موائم المزوّد، تحجز EstaCoda ذريًا حدًا أقصى محافظًا في كل نطاق منطبق. وبعد اكتمال الاستدعاء، تسوّي الحجز مقابل الاستخدام المسجّل. يمنع ذلك العمال أو العمليات المتزامنة من إنفاق السعة المتبقية نفسها بصورة مستقلة.

القيمة تقدير تشغيلي، وليست وعدًا بمطابقة فاتورة المزوّد بدقة. قد تختلف تغييرات الأسعار، وبدلات الاشتراك، والرصيد، والشرائح، والتقريب، وبيانات الاستخدام غير المكتملة عن تقدير EstaCoda. تميّز واجهات الاستخدام بين التقدير المكتمل، والحد الأدنى المعروف، والتكلفة غير المتاحة بدل اعتبار التسعير المفقود مجانيًا.

عندما يبلغ الإنفاق المسجّل مع السعة المحجوزة `warningThresholdPercent`، تسجل EstaCoda تحذيرًا دائمًا واحدًا لذلك النطاق وتعرضه عبر التقدم التفاعلي أو البعيد. يمنع بلوغ السقف إرسال طلب مزوّد جديد. كما لا يُرسل الطلب إذا لم يتسع الجزء غير المحجوز المتبقي للحد الأقصى المحافظ لذلك الطلب؛ وفي هذه الحالة توضّح EstaCoda أن العمل النشط يحجز السعة بدل الادعاء بأن الحد قد أُنفق. ويحتفظ العمل الجاري بحجزه إلى أن يكتمل أو يُسترد بأمان.

إذا كان هناك حد مالي منطبق ومفعّل ولم يكن للنموذج المحدد تسعير يمكن التحقق منه، ترفض EstaCoda الطلب قبل الإرسال لأنها لا تستطيع فرض السقف. كما ترفض الطلب الذي لا يمكن تحديد حد محافظ له. عند عدم وجود حد منطبق، قد يعمل مسار بلا تسعير، لكن الاستخدام المالي قد يظهر جزئيًا أو غير متاح.
