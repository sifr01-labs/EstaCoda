...
يستأنف `--continue` / `-c` صراحةً آخر جلسة CLI للملف الشخصي المحدد ومساحة العمل الحالية. يبدأ `estacoda` المجرّد جلسة جديدة. تصلح علامة الاستئناف لتشغيل تفاعلي أو prompt أحادي التشغيل أو أمر شرطة مائلة، ولا تصلح لأوامر المشغّل المستقلة التي تُوجّه قبل تشغيل runtime للمحادثة.
...
أُزيل الأمر `estacoda init`. يجهّز الإعداد الأول الحالة تلقائيًا؛ استخدم `estacoda doctor --fix` لإصلاحات الحالة المحلية الآمنة.
...
استخدم `estacoda verify vision` لإثبات مسار الرؤية الثنائي اللغة في الملف الشخصي المحدد. تعمل سلاسل المسارات المحلية بالكامل مباشرةً، ويتطلب المسار المحدد المستضاف أو أي مسار احتياطي مستضاف محتمل الخيار `--consent-hosted`. الأمر للقراءة فقط ويعيد الرمز 0 فقط عند نجاح نتيجة المسار المهيأ.
...
يبدأ `estacoda` المجرّد جلسة CLI جديدة عند كل تشغيل. يكون الاستئناف صريحًا دائمًا.

```bash
estacoda --continue                      # استئناف آخر جلسة لهذا الملف الشخصي/مساحة العمل
estacoda sessions                       # اختيار جلسة حديثة واستئنافها تفاعليًا
estacoda sessions open <session-id>     # استئناف جلسة معروفة عبر المعرّف
estacoda sessions list                  # الجلسات الأخيرة مع الأسطح المرتبطة
estacoda sessions show <session-id>     # تفاصيل الجلسة + مؤشرات الأسطح
estacoda sessions diagnose <session-id> # تشخيص تنفيذ محدود ومنقّح
...
**الحالة المُعدّلة:** قاعدة بيانات جلسات SQLite (`~/.estacoda/sessions.sqlite`) وفهرس استئناف CLI بالإصدار 2 (`~/.estacoda/cli-sessions.json`) عند تشغيل runtime للمحادثة أو تبديلها.

**حدود الملف التعريف:** تُربط أهداف الاستئناف ومؤشراته بالملف الشخصي المحدد ومساحة العمل الحالية ويُعاد التحقق منها. تُرفض الجلسات المنتهية والداخلية والفرعية والعابرة للملفات الشخصية أو مساحات العمل. يظل `sessions recall` مُقيّدًا بالملف الشخصي النشط ومساحة العمل عند توفر البيانات الوصفية.

**تشخيص التنفيذ:** يكون `sessions diagnose` للقراءة فقط ويُصرَّح له وفق الملف الشخصي المحدد. يلخّص استخدام الموفّر ونشاط الأدوات وتقدّم Mission وأدلة التنفيذ وإيصالات انتقال المصادقة المحمية، من دون طباعة prompts أو الرسائل أو حمولات الأدوات أو التسميات المحمية أو محتوى المتصفح أو URLs أو المسارات الخاصة أو الأسرار أو المعرّفات المشتقة من tokens. لا تُعرض فجوة موفّر بين التسليم والإرسال بالقيمة `no` إلا عندما يثبتها إيصال تسليم محمي ذري؛ وإلا تبقى `unknown`.

**سلوك أداة الاختيار:** في طرفية تفاعلية، يعرض `estacoda sessions` المجرّد ما يصل إلى 20 جلسة جذر قابلة للاستئناف وموجّهة للمستخدم ولها نشاط مستخدم، من الملف الشخصي المختار ومساحة العمل الحالية. يعرض رقم الجلسة ووصفًا منقحًا، ويعرض للصف المحدد وقت البدء وآخر نشاط وسطح المنشأ الثابت. يستأنف Enter الجلسة عبر فحوص الإعداد وثقة مساحة العمل المعتادة، ويلغي Escape الاختيار. يبقى `sessions list` عرض المشغّل غير التفاعلي.

**سلوك `--continue`:** يمكن دمج العلامة مع تشغيل تفاعلي أو prompt أحادي التشغيل أو أمر شرطة مائلة. تُرفض مع أوامر CLI التشغيلية المستقلة التي تُوجّه قبل تشغيل runtime للمحادثة. تفشل المؤشرات المحددة النطاق المفقودة أو التالفة أو القديمة بصورة مغلقة وتوجّه المستخدم إلى `estacoda sessions`.

**أنماط الفشل:**
- يستخدم `sessions open <session-id>` رسالة عامة لعدم توفر الأهداف المفقودة أو الخارجة عن النطاق، حتى لا يكشف ملفات شخصية أو مساحات عمل أخرى.
