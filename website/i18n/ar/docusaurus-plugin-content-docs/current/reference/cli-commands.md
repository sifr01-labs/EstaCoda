...
**السلوك:** يتطلب `begin` مساحة عمل موثوقة وينشئ خطوة وكيل واحدة بصلاحيات متحفظة. من دون `--session` تكون المهمة الجذرية مملوكة للنظام صراحةً ولا تُنشأ جلسة مخفية. يعرض `show` تقدماً محدوداً، وأعداد التنفيذ والانتظار، واكتمال بيانات الاستخدام والتسعير، وحالة الثقة، وعدد النتائج، وحالة المضيف الخلفي. تغيّر أوامر `pause` و`resume` و`cancel` و`retry` الصريح الحالة الدائمة. يسرد `result` مقابض مبهمة وملخصات من دون الأجسام الكاملة أو المسارات المحلية. لا يغيّر تجاوز `--profile` الخاص بالأمر الملف الشخصي النشط.
