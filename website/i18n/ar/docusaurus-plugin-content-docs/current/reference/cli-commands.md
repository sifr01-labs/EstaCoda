...
يستأنف `--continue` صراحةً آخر جلسة CLI للملف الشخصي المحدد ومساحة العمل الحالية. يبدأ `estacoda` المجرّد جلسة جديدة. تصلح علامة الاستئناف لتشغيل تفاعلي أو prompt أحادي التشغيل أو أمر شرطة مائلة، ولا تصلح لأوامر المشغّل المستقلة التي تُوجّه قبل تشغيل runtime للمحادثة.
