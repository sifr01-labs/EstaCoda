...
**سلوك الرؤية:** تقبل `vision.analyze` الحقل المتوافق `path` لصورة واحدة، أو `paths` من صورتين إلى أربع صور في وضع `compare`، إضافة إلى `prompt` الاختياري، ومستوى التفاصيل `low` و`standard` و`high`، وعمق الإخراج `concise` و`standard` و`detailed`. يؤدي إدخال عدة صور إلى اختيار `compare` عند إغفال الوضع. لا تقبل إلا ملفات عادية داخل الجذور المسموح بها بعد التحقق من المسار الحقيقي، وتكتشف MIME من البايتات السحرية، وتصحح الاتجاه، وتحذف metadata، وتضع حدودًا مستقلة لقراءة المصدر والأبعاد والبكسلات والذاكرة المفكوكة وإطارات الحركة وبكسلاتها وحجم الخرج لكل صورة ومجموع الأحجام والتزامن. النص المرئي داخل الصور محتوى غير موثوق ولا يُتبع كتعليمات.
