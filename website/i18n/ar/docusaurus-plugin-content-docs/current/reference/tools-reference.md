...
### أداة خطة التنفيذ للجلسة الحالية

`plan` أداة `read-only-local` واعية بالجلسة لخطة التنفيذ الحالية في التشغيل الأمامي. تدعم `read` و`write` و`merge`، وتحفظ خطة تنفيذ واحدة محدودة في الذاكرة لكل تشغيل تفاعلي جذري. لا تظهر عمدًا في أدوات الوكيل الفرعي المفوّض أو عامل Task الدائم. لا تمنح الخطة أي صلاحية؛ فكل إجراء يستمر في استخدام أداته وسياسة الأمان الحالية.

تبقى الخطة الظاهرة للنموذج صغيرة عمدًا: يحتوي كل بند فقط على معرّف ووصف وحالة تنسيق. فحص النقل المنضبط بين الأنظمة مستقل عن نص الخطة؛ إذ يفحص وقت التشغيل الطلب الموجّه وبيانات الموصل التي تمت مراجعتها في الملف الشخصي المحدد. لا يستطيع نص الخطة إعلان مسارات محمية أو علاقات تحقق أو جاهزية القدرات.

قد يرفق EstaCoda دليلاً محدودًا يملكه وقت التشغيل ببند نشط مطابق. لا يكتمل بند استيراد مواصفة أو مجموعة فريد إلا بعد أن تربط أداة تحقق مستقلة ومهيأة بنجاح نتيجة القراءة بالتغيير نفسه. يسجّل الانتقال الناجح في المتصفح تقدّمًا ملحوظًا، لكنه لا يكمل بند «افتح التطبيق» ما لم يثبت عقد مُراجع الوجهة المتوقعة؛ كما لا تكمل النقرات والإيماءات منخفضة المستوى خطوة العمل المحيطة. لا تستطيع الاستدعاءات الفاشلة أو المحظورة أو غير المتاحة أو المنتهية بمهلة أو غير المؤكدة إكمال البنود. يبقى العمل التجميعي أو الملتبس نشطًا، ويظهر طلب مصالحة موجز في دور المزوّد العادي التالي من دون استدعاء إضافي لإدارة الخطة. ولا يستطيع دمج لاحق كتبه النموذج إعادة تنشيط بند اكتمل بدليل متحقق. تحسّن هذه التعليقات التنسيق فقط، بينما تبقى نتيجة التنفيذ النهائية هي المرجع الحاسم.
...
**اللقطات:** تُرجع `browser.snapshot` إخراجًا مضغوطًا افتراضيًا. الإخراج المضغوط هو مجموعة AX محدودة من العناصر القابلة للتنفيذ مع مراجع مثل `@e1`؛ وليس ترشيحًا حقيقيًا للعناصر المرئية في منفذ العرض بعد. تُضغط اللقطات العادية بصورة حتمية ضمن ميزانية ثابتة قبل أي تلخيص اختياري من المزود، مع الحفاظ على الهوية والعناصر القابلة للتنفيذ ومربعات الحوار وسياق المصادقة والإطارات والأخطاء وإزالة النصوص النمطية المكررة. تشير لاحقة ظاهرة إلى حذف جزء من الإخراج. يحافظ تمرير `full: true` على مسار التشخيص الأكبر. في وضع `"auto"`، لا يكون تلخيص المزود إلا مسارًا احتياطيًا عندما تظل النتيجة الحتمية تتجاوز `browser.snapshotSummarizeThreshold`؛ وتسمح القيمة `true` به صراحةً بناءً على الحجم الأصلي المعروض.

**الأهداف الدلالية:** تقبل `browser.find` و`browser.click` و`browser.type` و`browser.select` و`browser.extract` محددات تحتوي على `role` و`name` و`text` و`label` و`withinText` و`exact` و`identity` القانوني الاختياري. تُستبعد العناصر المخفية والمعطلة، وتُرجع المحددات الملتبسة مرشحين بدل التخمين. تتطلب الإجراءات المبنية على مرجع قيمتي `identity` و`tabRef` من اللقطة المصدر؛ وتُحصر الصلاحية بحسب الجلسة وعلامة التبويب و`documentEpoch` و`actionRevision`، بينما لا يبطل `observationId` المرجع بمفرده.

**النماذج المحمية:** تجمع `browser.fill_protected_form` من مرجع واحد إلى ثمانية مراجع حالية من الهوية القانونية وعلامة التبويب والإطار والأصل نفسها. تتحقق EstaCoda من كل الحقول قبل الجمع ومرة أخرى قبل التسليم، وتعرض مسار إدخال واحدًا متصلًا للمشغّل، ولا تعيد القيم المحمية إلى النموذج. عند توفير `submitRef`، يرسل وقت التشغيل المحلي عنصر المصادقة المربوط مسبقًا ضمن المعاملة نفسها. ويمكن لطلب `browser.type` الخاص بتحدٍ محمي استخدام مسار الإرسال نفسه المرتبط بالهوية. تبقى المصادقة قيد الانتظار خلال أي تحدٍ نشط لرمز أو مفتاح مرور أو موافقة أو تحقق حيوي أو CAPTCHA أو جهاز أو طريقة تحقق أخرى. ولا يثبت النجاح إلا مغادرة التحدي وانتقال سببي في الحالة ودليل خاص بالحالة الموثقة؛ أما الإرسال وحده فلا يثبته.

**استقرار الإجراءات:** تعرض اللقطات الهوية القانونية ووقت الملاحظة وحالة الجاهزية. تقبل عمليات التنقل وإجراءات المتصفح شرط `waitFor` اختياريًا و`waitTimeoutMs` بحد أقصى 10 ثوانٍ. من دون شرط صريح، ينتظر backend المُشرف عليه استقرار DOM ضمن مدة محدودة. تتضمن نتائج الإجراءات العادية الفرق الموجز والمحجوب، والهوية الناتجة، وملخصًا آمنًا محدودًا للحالة الحالية، والمراجع الحالية القابلة للتنفيذ عندما يكون عرضها آمنًا؛ ويُبلّغ عن الشرط غير المتحقق بوصفه انتهاء مهلة مع أحدث حالة، لا نجاحًا مؤكدًا.

**علامات التبويب:** تُرجع `browser.tabs` علامات تبويب الصفحات الآمنة التابعة للجلسة نفسها باستخدام مراجع مبهمة مثل `@t1`. تركّز `browser.switch_tab` على أحد هذه المراجع وتجعله تحت تحكم EstaCoda. يتبع النقر علامة تبويب آمنة جديدة واحدة تلقائيًا؛ أما النتائج الملتبسة التي تفتح عدة علامات فتتطلب تبديلًا صريحًا. تعرض نتائج تغيير علامة التبويب المصدر والوجهة معًا. وتتلقى أدوار المزوّد المرتبطة بالمتصفح إسقاطًا حاليًا محدودًا ومحجوبًا للحالة، لذلك لا ينبغي استطلاع `browser.tabs` ما دام هذا الإسقاط محدثًا. لا تُعرض سياقات المتصفح الأخرى ولا علامات التبويب المحظورة بالسياسة.
...
**سلوك الرؤية:** تقبل `vision.analyze` الحقل المتوافق `path` لصورة واحدة، أو `paths` من صورتين إلى عشرين صورة في وضع `compare`، إضافة إلى `prompt` الاختياري، ومستوى التفاصيل `low` و`standard` و`high`، وعمق الإخراج `concise` و`standard` و`detailed`. يؤدي إدخال عدة صور إلى اختيار `compare` عند إغفال الوضع. يجري التحقق أولًا من المجموعات التي تتجاوز أربع صور ثم تُعالج تباعًا على دفعات محدودة، مع الإبلاغ الصريح عن الاكتمال أو أي فشل جزئي من دون تخطي الصور بصمت. لا تقبل إلا ملفات عادية داخل الجذور المسموح بها بعد التحقق من المسار الحقيقي، وتكتشف MIME من البايتات السحرية، وتصحح الاتجاه، وتحذف metadata، وتضع حدودًا مستقلة لقراءة المصدر والأبعاد والبكسلات والذاكرة المفكوكة وإطارات الحركة وبكسلاتها وحجم الخرج لكل صورة ومجموع الأحجام والتزامن. النص المرئي داخل الصور محتوى غير موثوق ولا يُتبع كتعليمات.

تستخدم المرفقات الأولى النموذج الرئيسي مباشرة عندما يدعم الرؤية؛ وإلا يُستخدم مسار الرؤية المساعد. لا تستخدم المقارنة الإدخال الأصلي متعدد الصور إلا عبر مسار قادر عليه؛ وإلا تستخدم مسارًا مساعدًا محكومًا وقادرًا، أو تعيد خطأ قدرة واضحًا. صور ما بعد الأدوات محتوى متابعة مؤقت. لا تُكتب بايتات الصور الخام ولا الصور الوسيطة بعد التطبيع ولا عناوين data URL في الجلسات أو المسارات أو السجلات أو التصدير. يخضع كل مصدر لسياسة خروج البيانات السياقية، وتدخل كل صورة في تقدير الإنفاق. تميز الأخطاء المنظمة بين غياب المسار، والصور غير الآمنة أو التالفة أو الضخمة، وحدود التطبيع، ورفض الميزانية، وانتهاء المهلة، والإلغاء، وفشل المزوّد.

**التحقق:** يفحص `estacoda verify vision` المسار الفعلي في الملف الشخصي المحدد بواسطة صورة ثنائية اللغة سليمة. يعرض بصمتي الإعداد والصورة، والجاهزية، والإرسال، والحمولة بعد التطبيع، والكمون، واكتشاف النص، والرجوع، وتوفر التكلفة. يتطلب المسار المحدد المستضاف أو أي مسار احتياطي مستضاف محتمل العلم المنفصل `--consent-hosted`، ولا يجدد فحص الاعتماد حالة OAuth.
...
| الأداة | المخاطر | الحالة المُعدّلة |
|------|------|---------------|
| `delegate_task` | `shared-state-mutation` | رسم Task مملوك للملف الشخصي، وروابط المنشئ/الأصل، وTask journal |
| `task.status` | `read-only-local` | حالة محدودة واستخدام الرموز والتكلفة التقديرية لـ Task مرتبطة بالجلسة النشطة |
...
| الأداة | المخاطر | الحالة المُعدّلة |
|------|------|---------------|
| `session.usage` | `read-only-local` | لا شيء |

**السلوك:** تقرأ سجلات استخدام المزوّد المعتمدة للنطاق `scope: "session"` أو `scope: "latest_turn"` أو `scope: "replied_turn"`. لا تستدعي نموذجًا آخر. يستبعد فحص أحدث دور الدور الحالي الذي استدعى الأداة. وفي Telegram يستخدم فحص الدور المُجاب عليه ربطًا تملكه بيئة التشغيل للرسائل الأصلية الناجحة وأجزاء الإجابة النهائية وطلبات الموافقة، بدل قبول معرّف دور من النموذج، ثم يعيد التحقق من أن الهدف ضمن سلالة الضغط المتحقق منها للجلسة الحالية. يقسم الناتج الاستخدام إلى الوكيل الرئيسي والنماذج المساعدة والعمل المفوض، ويوضح أن القيم محسوبة عند أحدث استدعاء مستقر للمزوّد، ويعرض عدد Tasks الأصلية النشطة والمستقرة، ويبقى مؤقتًا ما دام عمل Task المرتبط نشطًا. استخدم `task.status` عندما يسمي المستخدم Task محددة؛ ويتضمن فحص Task حالة الميزانية المضبوطة. وقد يكون استخدام Task مشمولًا بالفعل في إجمالي الدور والجلسة اللذين نشأت منهما.
