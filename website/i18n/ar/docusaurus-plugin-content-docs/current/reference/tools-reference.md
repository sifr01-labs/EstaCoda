...
**السلوك:** تقرأ سجلات استخدام المزوّد المعتمدة للنطاق `scope: "session"` أو `scope: "latest_turn"` أو `scope: "replied_turn"`. لا تستدعي نموذجًا آخر. يستبعد فحص أحدث دور الدور الحالي الذي استدعى الأداة. وفي Telegram يستخدم فحص الدور المُجاب عليه ربطًا تملكه بيئة التشغيل بين الرسالة المسلّمة والدور، بدل قبول معرّف دور من النموذج، ثم يعيد التحقق من أن الهدف ضمن سلالة الجلسة الحالية. يقسم الناتج الاستخدام إلى الوكيل الرئيسي والنماذج المساعدة والعمل المفوض، ويبقى مؤقتًا ما دام عمل Task المرتبط نشطًا. استخدم `task.status` عندما يسمي المستخدم Task محددة. وقد يكون استخدام Task مشمولًا بالفعل في إجمالي الدور والجلسة اللذين نشأت منهما.
