...
**سلوك الرؤية:** تقبل `vision.analyze` الحقول `path` و`prompt` الاختياري، والأوضاع `describe` و`ocr` و`document` و`chart` و`screenshot`، ومستوى التفاصيل `low` و`standard` و`high`، وعمق الإخراج `concise` و`standard` و`detailed`. لا تقبل إلا ملفات عادية داخل الجذور المسموح بها بعد التحقق من المسار الحقيقي، وتكتشف MIME من البايتات السحرية، وتصحح الاتجاه، وتحذف metadata، وتحد البايتات والأبعاد والبكسلات والذاكرة المفكوكة وإطارات الحركة وحجم الخرج والتزامن. النص المرئي داخل الصور محتوى غير موثوق ولا يُتبع كتعليمات.

تستخدم المرفقات الأولى النموذج الرئيسي مباشرة عندما يدعم الرؤية؛ وإلا يُستخدم مسار الرؤية المساعد. صور ما بعد الأدوات محتوى متابعة مؤقت. لا تُكتب بايتات الصور الخام ولا عناوين data URL في الجلسات أو المسارات أو السجلات أو التصدير. تبقى الاستدعاءات المستضافة خاضعة لموافقة خروج البيانات السياقية وميزانيات الإنفاق. تميز الأخطاء المنظمة بين غياب المسار، والصور غير الآمنة أو التالفة أو الضخمة، وحدود التطبيع، ورفض الميزانية، وانتهاء المهلة، والإلغاء، وفشل المزوّد.
