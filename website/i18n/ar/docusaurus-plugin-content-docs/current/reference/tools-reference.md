...
تعمل Step التركيب باتجاه المنع: تحصل على مقابض Results محدودة وصلاحية `task.result.read` فقط، ولا يمكنها التفويض، ولا تعمل قبل اكتمال جميع العاملين. يؤدي فشل عامل إلى تخطي التركيب وبقاء Task بالحالة `partial`. وعند النجاح تُوسم Result الخاصة بالتركيب بأنها النتيجة الرئيسية، بينما تبقى Results الوسيطة متاحة عبر مقابض opaque.
...
تحافظ سياسة Step الافتراضية على أدوات `read-only-local` و`read-only-network` المرئية للأصل، ثم تطبق الحظر الدقيق وحظر البادئات ومجموعات الأدوات المستبعدة وطلبات الأدوات أو مجموعات الأدوات بوصفها تضييقًا إضافيًا. لا يمكن لـ Steps بدور `worker` التفويض. ولا تحتفظ Steps بدور `orchestrator` بأداة `delegate_task` إلا بوجود صلاحية إنشاء طفل محفوظة وعمق متبقٍ. يجب أن تحافظ Task الفرعية المرتبطة على workspace الأصل وأن تحصل على صلاحية وميزانية أضيق.
