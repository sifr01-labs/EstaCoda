...
تستخدم `browser.snapshot` لقطة مضغوطة افتراضيًا. اللقطات المضغوطة هي مجموعة AX محدودة من العناصر القابلة للتنفيذ، وليست ترشيحًا حقيقيًا للعناصر المرئية في منفذ العرض بعد. تضغط EstaCoda اللقطات العادية بصورة حتمية قبل إرجاعها إلى النموذج، مع إعطاء الأولوية للهوية ومربعات الحوار والتنبيهات والمراجع القابلة للتنفيذ والنماذج وأخطاء التحقق وسياق المصادقة والإطارات وأخطاء المتصفح، وإزالة التنقل المتكرر والنصوص النمطية المكررة. تشير لاحقة ظاهرة إلى حذف جزء من الإخراج. يحافظ تمرير `full: true` على مسار التشخيص الأكبر.

تستخدم EstaCoda فحصًا موحدًا لقابلية التفاعل عند إنشاء المراجع والبحث والاستخراج والتحقق المسبق واللحظة الأخيرة قبل تنفيذ الإجراء. تستبعد العناصر المنفصلة أو المخفية أو الخاملة أو المعطلة أو ذات المساحة الصفرية أو المحجوبة بنافذة حوار نشطة. تبقى العناصر داخل نافذة الحوار النشطة والعناصر الصالحة خارج منفذ العرض قابلة للاستخدام، ولا يؤدي استبعاد عنصر إلى حذف نص الصفحة العادي أو معلومات التشخيص.

تتضمن اللقطات هوية قانونية، ووقت الملاحظة، وحالة جاهزية الصفحة. تفصل الهوية بين استبدال المستند (`documentEpoch`) وتغيّر المراجع القابلة للتنفيذ (`actionRevision`) واللقطات الفردية (`observationId`). تنتظر إجراءات مثل `browser.navigate` و`browser.click` و`browser.type` و`browser.press` و`browser.back` استقرار DOM لفترة قصيرة افتراضيًا، ثم تُرجع فرقًا موجزًا والهوية الناتجة وملخصًا آمنًا للحالة الحالية والمراجع القابلة للتنفيذ عندما يكون ذلك آمنًا. يمكن تمرير `waitFor` لانتظار جزء من URL، أو نص في الصفحة، أو عنصر بحسب `role`/`name`، أو مربع حوار، أو استقرار DOM، مع `waitTimeoutMs` حتى 10 ثوانٍ. إذا لم يتحقق الشرط، تُرجع EstaCoda أحدث حالة وتصف النتيجة بأنها انتهت مهلتها بدل الادعاء بالنجاح. استخدم `browser.snapshot` عندما تحتاج صراحةً إلى لقطة مفصلة لحالة الصفحة الحالية.

يفضل استخدام المحددات الدلالية للتفاعل. تستطيع `browser.find` و`browser.click` و`browser.type` و`browser.select` و`browser.extract` استهداف العناصر عبر `role` أو`name` أو`text` أو`label` أو النص المحيط `withinText`؛ مثل زر “View product” داخل بطاقة “Security MTN OAuth V1”. تُستبعد العناصر غير القابلة للتفاعل، وتُرجع المطابقات الملتبسة قائمة مرشحين بدل اختيار أحدها تلقائيًا. عند استخدام مرجع مثل `@e1`، مرر أيضًا `identity` القانوني و`tabRef` من اللقطة التي أنشأت المرجع. ترفض EstaCoda المراجع التابعة لحالة مستند/إجراء قديمة أو لعلامة تبويب أخرى قبل إرسال الإجراء.

عندما تطلب الصفحة عدة قيم محمية مترابطة، مثل البريد الإلكتروني وكلمة المرور، تستخدم EstaCoda مسار نموذج آمن واحدًا. تعرض كل وجهة متحققًا منها بالتتابع، وتُبقي القيم خارج سياق النموذج، ثم تعيد التحقق من النموذج كاملًا وتملأ الحقول، ويمكنها إرسال عنصر تسجيل دخول مربوط مسبقًا ضمن المعاملة المحمية نفسها.

تُبقي EstaCoda تسجيل الدخول الكلي قيد الانتظار ما دامت الصفحة الناتجة تطلب تحديًا، حتى لو ظهرت فيها أيضًا عناصر تبدو خاصة بالحساب. وقد يكون التحدي رمز تحقق بخطوتين، أو مفتاح مرور أو مفتاح أمان، أو تحققًا حيويًا، أو CAPTCHA، أو موافقة عبر إشعار، أو تأكيد جهاز، أو طريقة تحقق أخرى. تواصل إجراءات إعادة الإرسال والمحاولة خطوة Mission نفسها. ولا تعلن EstaCoda النجاح إلا بعد مغادرة التحدي عبر انتقال سببي في المتصفح وظهور دليل خاص بالحالة الموثقة؛ فإرسال بيانات الاعتماد أو التحدي وحده لا يثبت النجاح. ولا يُحتسب وقت انتظار إدخالك للقيم المحمية من ميزانية وقت تشغيل المزوّد المستقل.
...
في وضع `"auto"`، يحدث الضغط الحتمي أولًا، ولا يُستخدم نموذج مساعد إلا إذا ظلت النتيجة المضغوطة تتجاوز العتبة. تسمح القيمة `true` صراحةً بالتلخيص بناءً على الحجم الأصلي المعروض، بينما تمنع القيمة `false` استدعاء مزود التلخيص. تُحجب عناوين URL التي تحتوي أسرارًا والقيم الحساسة قبل استدعاءات المزود.
...
في المتصفح المحلي المُشرف عليه، يستطيع EstaCoda العمل عبر علامات التبويب دون مطالبة النموذج بتفسير بيانات CDP الخام:

- تعرض `browser.tabs` علامات تبويب الصفحات الآمنة في جلسة المتصفح المعزولة الحالية، وتحدد علامة التبويب التي يتحكم فيها EstaCoda حاليًا.
- تركّز `browser.switch_tab` على علامة تبويب عبر مرجع مبهم مثل `@t2` وتجعلها هدف اللقطات والإجراءات اللاحقة.
- بعد النقر، يتبع EstaCoda علامة التبويب الجديدة تلقائيًا فقط عندما تظهر علامة تبويب آمنة جديدة واحدة بالضبط. إذا فُتحت عدة علامات، يبقى في العلامة الحالية ويعرض مراجعها ليختار الوكيل صراحةً.

أثناء العمل في المتصفح، يزوّد EstaCoda النموذج أيضًا بملخص صغير للحالة الحالية يتضمن حالة الجلسة المباشرة، وعلامة التبويب التي يتحكم فيها، وعلامات التبويب الآمنة، وهوية الصفحة القانونية وجاهزيتها، وآخر إجراء للمتصفح. يتجاوز هذا الملخص تفاصيل المتصفح الأقدم في المحادثة ويقلل تكرار فحص علامات التبويب أو اللقطات. إذا تنقلت أو غيّرت المتصفح الذي يتحكم فيه EstaCoda يدويًا، يُحدَّث الملخص عند بداية الخطوة التالية المرتبطة بالمتصفح. ويعرض تغيير علامة التبويب المصدر والوجهة معًا.

إذا كرر الوكيل اللقطة أو قائمة علامات التبويب الناجحة نفسها دون تقدم، تعطيه EstaCoda توجيه استرداد واحدًا ثم توقف الحلقة بعد الملاحظة الثالثة غير المتغيرة. يؤدي تغير الصفحة أو قائمة علامات التبويب أو إجراء يغير حالة المتصفح إلى إعادة ضبط الحارس، بينما لا يُحتسب الإجراء الذي يبلغ صراحةً عن `no-change` أو `timeout` تقدمًا.

تُستبعد علامات التبويب التابعة لجلسات متصفح EstaCoda الأخرى. وتُخفى العلامات المحظورة بسياسة أمان URL أو سياسة المواقع أو حماية بيانات التعريف السحابية أو كشف الأسرار، ولا يمكن اختيارها عبر `browser.switch_tab`.
