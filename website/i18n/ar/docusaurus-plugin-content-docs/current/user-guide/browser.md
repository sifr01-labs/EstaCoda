...
يفضل استخدام المحددات الدلالية للتفاعل. تستطيع `browser.find` و`browser.click` و`browser.type` و`browser.select` و`browser.extract` استهداف العناصر عبر `role` أو`name` أو`text` أو`label` أو النص المحيط `withinText`؛ مثل زر “View product” داخل بطاقة “Security MTN OAuth V1”. تُستبعد العناصر المخفية والمعطلة، وتُرجع المطابقات الملتبسة قائمة مرشحين بدل اختيار أحدها تلقائيًا. عند استخدام مرجع مثل `@e1`، مرر أيضًا `identity` القانوني و`tabRef` من اللقطة التي أنشأت المرجع. ترفض EstaCoda المراجع التابعة لحالة مستند/إجراء قديمة أو لعلامة تبويب أخرى قبل إرسال الإجراء.
