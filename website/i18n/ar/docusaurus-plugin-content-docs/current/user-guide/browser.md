...
تتضمن اللقطات مراجعة خاصة بالجلسة، ووقت الملاحظة، وحالة جاهزية الصفحة. تنتظر إجراءات مثل `browser.navigate` و`browser.click` و`browser.type` و`browser.press` و`browser.back` استقرار DOM لفترة قصيرة افتراضيًا، ثم تُرجع ملخصًا موجزًا للتغييرات. يمكن تمرير `waitFor` لانتظار جزء من URL، أو نص في الصفحة، أو عنصر بحسب `role`/`name`، أو مربع حوار، أو استقرار DOM، مع `waitTimeoutMs` حتى 10 ثوانٍ. إذا لم يتحقق الشرط، تُرجع EstaCoda أحدث حالة وتصف النتيجة بأنها انتهت مهلتها بدل الادعاء بالنجاح. استخدم `browser.snapshot` عندما تحتاج صراحةً إلى لقطة مفصلة لحالة الصفحة الحالية.

يفضل استخدام المحددات الدلالية للتفاعل. تستطيع `browser.find` و`browser.click` و`browser.type` و`browser.select` و`browser.extract` استهداف العناصر عبر `role` أو`name` أو`text` أو`label` أو النص المحيط `withinText`؛ مثل زر “View product” داخل بطاقة “Security MTN OAuth V1”. تُستبعد العناصر المخفية والمعطلة، وتُرجع المطابقات الملتبسة قائمة مرشحين بدل اختيار أحدها تلقائيًا. عند استخدام مرجع مثل `@e1`، مرر أيضًا `revision` و`tabRef` من اللقطة التي أنشأت المرجع. ترفض EstaCoda المراجع القديمة أو التابعة لتبويب آخر قبل إرسال الإجراء.
