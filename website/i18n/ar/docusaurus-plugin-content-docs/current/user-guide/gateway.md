...
- معالجة مهام إنهاء الجلسات الدائمة للملف الشخصي المحدد حتى لا يؤخر تنظيم الذاكرة عند نهاية الجلسة بدء الجلسة التالية أو خروج CLI.
...
عندما يستطيع الإعداد تثبيت خدمة البوابة المدارة وتشغيلها، يكون عنوان المطالبة `EstaCoda Gateway`.

يعرض onboarding الأول الخدمة كلما كان إنهاء الذاكرة في الخلفية مفعّلًا، بما في ذلك إعدادات CLI التي لا تضبط أي قناة. ويعرضها Setup Editor للمستخدم الحالي عندما يكوّن ذلك التشغيل أول قناة جاهزة جديدة.
...
في Setup Editor للمستخدم الحالي، لا تظهر المطالبة للتغييرات غير الخاصة بالقنوات أو عند وجود قناة جاهزة مسبقًا. ولا يعرضها أي مسار إعداد عندما تكون خدمة بوابة مدارة مثبتة أو نشطة بالفعل.
...
| WhatsApp | تعمل عبر bridge معزول؛ تتطلب `experimental: true` لأن الواجهة غير رسمية |
...
يمكن للصف نفسه أن يحمل موافقات إعداد قدرات Python المُدارة. إذا احتاجت مهارة محددة إلى قدرة مسجلة مطلوبة لكنها غير مثبتة، تستطيع البوابة إرسال مطالبة موافقة أصلية في القناة، مثل أزرار Install/Deny في Telegram. هذه الموافقة تخص معرف قدرة ومجموعات محددة، وليست أمر shell عشوائيًا.
...
- موافقات إعداد Python المُدارة تثبّت حزم القدرات المسجلة فقط، وتبطل Runtime cache للجلسة، ثم تعيد تشغيل الرسالة الأصلية بعد الموافقة.
...
```bash
estacoda gateway status      # الحالة الكاملة: القنوات، الموافقات، Cron، إنهاء الذاكرة، مدير الخدمة
...
يتضمن `gateway status` أعداد إنهاء الذاكرة الخاصة بالملف الشخصي: `pending` و`running` و`retrying` و`failed`. يحتوي الطابور على حدود الرسائل وبيانات تشغيلية محدودة، لا نص المحادثة. تستنزف خدمة البوابة المدارة العاملة الطابور في الخلفية عادةً؛ وتبقي الخدمة المتوقفة المهام محفوظة حتى تبدأ البوابة مرة أخرى.
