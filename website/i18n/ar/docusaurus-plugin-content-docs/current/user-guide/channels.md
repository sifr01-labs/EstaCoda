...
| تجميع النص الوارد السريع | `default-on, bounded` |
...
- تُجمع النصوص العادية السريعة من الحساب والمحادثة/الموضوع والمرسل أنفسهم، وتُفصل بسطر فارغ قبل دور واحد
...
يجمع Telegram أجزاء النص العادية لمدة `1500ms` افتراضيًا، بحدود `10` رسائل و`8000` حرف. تعيد عمليات التفريغ عند بلوغ الحدود التحكم إلى polling قبل اكتمال دور runtime، بينما ينتظر الإغلاق السليم للبوابة أعمال التفريغ المملوكة لها.

تتجاوز الأوامر وcallback queries ومسارات الربط/التفويض والمرفقات والألبومات التجميع. يبقى الألبوم دور مرفقات متعدد الصور واحدًا، ويظل وصف الألبوم هو prompt الخاص به، ولا يُدمج النص التالي مع الألبوم. اضبط `channels.telegram.textDebounceMs` على `0` للإرسال الفوري. لا تغير هذه الميزة إيقاع polling في Telegram أو طابور FIFO عند الانشغال.

### دمج النص الاختياري في طابور الانشغال

يمكن لكل قناة تفعيل دمج محدود لنهاية طابور FIFO عبر `busyTextCoalescing`، لكنه لا يعمل إلا مع `busyPolicy: "queue"`. يدمج النصوص العادية المتجاورة فقط عند تطابق الجلسة الأساسية والمرسل، ويحافظ على موضع العنصر الحالي في الطابور وعلى معرفات الرسائل المكوّنة وأوقات استلامها. تبقى الأوامر وcallbacks والموافقات والمرفقات والرسائل الصوتية والوسائط ومسار الاستبدال عبر `interrupt` منفصلة.
...
عند بلوغ أي حد تأخذ الرسالة الجديدة موضع FIFO التالي، ويبقى سلوك امتلاء الطابور كما هو. تعرض أوامر حالة القناة والبوابة ما إذا كان هذا الخيار مفعّلًا.
...
تكون طوابير الانشغال محلية للعملية افتراضيًا. لاسترداد الرسائل المنتظرة المقبولة بعد إعادة التشغيل، اضبط `gateway.messageQueue.persistence` على `"sqlite"` للملف الشخصي المحدد. تحفظ البوابة الرسالة قبل الإقرار بها، ثم تطالب بها قبل التنفيذ. يمكن استرداد العمل pending بترتيب FIFO؛ أما العمل الذي تركه الانهيار claimed فيصبح uncertain ولا يُعاد تلقائيًا.

تتضمن الصفوف الدائمة نص المستخدم، وتوجيه القناة/الجلسة/المرسل، ووقت الاستلام، وmetadata محدودة، وأوصاف المرفقات المحلية. ولا تتضمن بيانات اعتماد القنوات أو بايتات المرفقات. يُعاد فحص التفويض الحالي، وثقة مساحة العمل، ونطاق الجلسة، وملفات المرفقات قبل الاسترداد.

استخدم `/status` في قناة مفوضة لفحص أعداد pending/claimed/uncertain للملف الشخصي كله. يمسح `/stop` صفوف الطابور الخاصة بتلك المحادثة فقط عندما لا يكون هناك دور نشط؛ أما `/stop` أثناء دور نشط فيلغي الدور ويحافظ على طابوره. وضع SQLite حساس أمنيًا ولا يَعِد بتنفيذ exactly-once أو at-least-once غير مشروط. اقرأ [عمليات البوابة](../operations/gateway-operations.md#طابور-الانشغال-الدائم) قبل تفعيله.
...
| `/usage` | عرض الاستخدام المسجل والتكلفة التقديرية للجلسة الحالية |
| `/usage last` | عرض الاستخدام المسجل لأحدث دور ظاهر مكتمل |
| `/usage task <task-id>` | عرض الاستخدام المسجل لـ Task مصرّح بها من الجلسة الحالية |
...
| `/memory ...` | فحص وإدارة تنظيم الذاكرة بسلوك مكافئ لـ CLI |
| `/cron` | عرض مهام cron |
| `/diagnostics` | تشغيل تشخيصات البوابة |

في Telegram، رُد بالأمر `/usage` على رسالتك الأصلية، أو أي جزء من إجابة EstaCoda النهائية، أو طلب موافقة، لفحص الدور الذي نشأت منه الرسالة من دون استدعاء نموذج. ويمكنك أيضًا الرد بلغة طبيعية، مثل «كم كلّفت هذه؟»، ليستخدم الوكيل نطاق استخدام الدور المُجاب عليه للقراءة فقط. معرّف الرد المخزّن مجرد مؤشر توجيه؛ إذ يُعاد التحقق من صلاحية الربط ضمن الملف الشخصي وحساب Telegram ونوع المحادثة والمحادثة والموضوع والمستخدم وسلالة الجلسة المتحقق منها. تنتهي صلاحيته بعد 90 يومًا، وتُرفض افتراضيًا الرسائل التي فشل تسليمها أو انتهت صلاحيتها أو لم تُربط أو لا تنتمي إلى السلالة نفسها. ولا يُضاف تذييل تكلفة تلقائي.
