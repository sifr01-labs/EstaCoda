...
بالنسبة إلى الصور، يعالج المسار الرئيسي الداعم للرؤية المرفقات الأولى مباشرة. يستخدم المسار الرئيسي النصي مسار `vision` المساعد، وتُمرر الصور المكتشفة لاحقًا بصورة مؤقتة عبر السياسة نفسها. تشترك `vision.analyze` والمرفقات الأولى و`browser.vision` وآثار الصور المولّدة في مسار الإرسال المحكوم نفسه. تُحل مراجع `artifact://...` المولّدة داخل ذاكرة الصور المؤقتة للملف الشخصي المحدد من دون كشف مسار محلي مخفي. تبقى الأوصاف العادية على المسار الرئيسي متعدد الوسائط، بينما يتولى المسار المتخصص المهيأ صراحةً تحليلات OCR والمستندات والمخططات ولقطات الشاشة والمقارنة. تقبل المقارنة من صورتين إلى عشرين صورة؛ ويجري التحقق من المجموعات التي تتجاوز أربع صور ومعالجتها تباعًا على دفعات محدودة، بينما تستقبل المحولات المخصصة مجهولة القدرة صورة واحدة في كل دفعة ما لم تعلن صراحة دعم الصور المتعددة. لا يضيف التوجيه التلقائي استدعاءً مساعدًا غير ضروري، ويمنع إيقاف تحليل الصور صراحةً التحليل حتى عبر نموذج رئيسي يدعم الصور. يجب أن يملك كل مسار أساسي أو احتياطي محوّلًا مسجلًا وقابلًا للتنفيذ، وبيانات مزوّد قابلة للتشغيل، ودعمًا للرؤية. معالجة الصور المستضافة خروج سياقي للبيانات: لا تكرر المرفقات الحالية والمراجع الصريحة ولقطات المتصفح والآثار المولّدة مطالبات الموافقة في الوضع التكيفي؛ بينما تطلب الملفات التي اكتشفها الوكيل موافقة، ويطلب الوضع الصارم موافقة، ويحظر `hostedProcessing: "local-only"` الإرسال المستضاف. يشتق وقت التشغيل مصدر كل صورة في المقارنة، وتُرفض محاولات الاجتياز أو الهروب عبر الروابط الرمزية من ذاكرة الصور المؤقتة.

تُطبّع استدعاءات الرؤية موارد الصورة وتحدّها قبل الإرسال، بما يشمل حدود كل صورة ومجموع البايتات وبكسلات الحركة، وتحجز ميزانية الجلسة/المهمة عبر كل الصور للمحاولات الأساسية والاحتياطية، وتعيد بيانات المسار والكمون والاستخدام والتطبيع والموارد المجمعة والرجوع الاحتياطي. تبقى الصور الوسيطة بعد التطبيع في الذاكرة فقط. يعيد غياب المسار أو المصدر غير الآمن أو تجاوز الحدود أو تعذر تسعير مسار خاضع لميزانية أو انتهاء المهلة أو الإلغاء أو نفاد سلسلة المزوّدين فشلًا منظمًا بدل التدهور الصامت إلى بيانات وصفية فقط.

استخدم `estacoda verify vision` للتحقق من المسار الفعلي، سواء كان تلقائيًا أو رئيسيًا أو مخصصًا مستضافًا أو محليًا أو متوافقًا مع OpenAI. تشغّل سلسلة المسارات المحلية بالكامل الصورة الثنائية اللغة مباشرةً، بينما يتطلب المسار المحدد المستضاف أو أي مسار احتياطي مستضاف محتمل الخيار `--consent-hosted` لأنه قد يرسل الصورة ويسبب تكلفة. التحقق للقراءة فقط ويعرض القدرة وجاهزية الاعتماد والإرسال والكمون والحجم بعد التطبيع واكتشاف النص والرجوع وتوفر التكلفة وبصمة الإعداد وهاش الصورة، من دون كشف البايتات أو الأسرار.

يمكن للمشرفين تشغيل مسار الإصدار المقيم من نسخة المصدر عبر `pnpm run eval:vision:live`، مع اشتراط `--consent-hosted` لأي سلسلة مسارات تتضمن وجهة مستضافة محتملة ووضع حد أقصى افتراضي للتعرض التقديري قدره `$1.00`. استخدم `--max-cost-usd <amount>` لتعيين حد صريح آخر لهذه الجولة. تنتج العملية تقريري JSON وMarkdown عن أخطاء OCR والحقائق المسندة والهلوسة والكمون والتكلفة وحجم الحمولة وملاحظة الرجوع وتكرار الموافقة، وتقارنها بحدود محفوظة. لا يفشل المسار بسبب التباين العادي داخل الحدود المحددة.
