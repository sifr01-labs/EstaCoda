...
عندما تنتهي الجلسة دلاليًا، يُضاف تنظيم الذاكرة إلى طابور خلفي دائم. يحدث ذلك مع `/new` و`/reset` و`/exit` في CLI، ومع `Ctrl+C` أثناء الخمول، ومع `/new` و`/reset` في القنوات المصرح بها، وبعد نجاح prompt أحادي التشغيل. أما `Ctrl+C` أثناء دور نشط فيلغي الدور الحالي فقط. تنظيف runtime بسبب تحديث الإعدادات، أو إخلاء cache، أو أعمال cron، أو `Runtime.dispose()` العام لا ينهي الجلسة ولا يضيف تنظيمًا إلى الطابور.

بدء الجلسة التالية أو الخروج ينتظر فقط كتابة قصيرة ودائمة في قاعدة البيانات، ولا ينتظر استخراج النموذج أو كتابة الذاكرة. إذا فشلت الإضافة، يظهر تحذير محدود من دون حجب الانتقال. تعالج خدمة البوابة المدارة المهمة في الخلفية للملف الشخصي المحدد، ويعرض الإعداد الأول الخدمة أيضًا لإعدادات CLI التي لا تحتوي على قناة. إذا كانت الخدمة متوقفة، تبقى المهمة محفوظة في `~/.estacoda/sessions.sqlite` وتعمل عند تشغيل بوابة ذلك الملف الشخصي لاحقًا.

تسجل كل مهمة حدًا ثابتًا للرسائل مع بيانات تشغيلية محدودة، ولا تخزن نسخة من النص. لذلك لا يشمل التنظيم رسائل لاحقة أضيفت إلى جلسة مستأنفة ويستخدم مساحة عمل الجلسة الأصلية. لا تُقدِّم إخفاقات الاستخراج أو تطبيق الذاكرة مؤشّر التنظيم إلى الأمام، ولذلك تعيد المحاولات المحدودة معالجة شريحة الرسائل نفسها. استخدم `estacoda memory status` أو `estacoda gateway status` لفحص الأعداد الخاصة بالملف الشخصي، واستخدم `estacoda memory finalization list` و`retry` و`prune` لفحص أكواد النتيجة/الخطأ المحدودة واسترداد المهام محليًا.
...
إدارة طابور الإنهاء متاحة في CLI المحلي فقط:
...
## Telemetry نتائج التفويض

نتائج التفويض منفصلة عن استرجاع نصوص الجلسات الفرعية وعن ذاكرة prompt canonical. تُسجل كـ telemetry تشغيلية في أحداث الجلسة وسجلات trajectory.

يمكن أن تتضمن telemetry المسجلة metadata محدودة مثل معرف جلسة الأب، ومعرف جلسة الطفل، والدور، والعمق، وفهرس المهمة، والحالة، والسبب، والطابع الزمني، واستخدام الرموز، ومعاينة محدودة للمهمة.

لا تكتب في `MEMORY.md`، ولا تخزن مخرجات الطفل الخام، أو prompts، أو transcripts، أو tool arguments، أو محتوى الملفات، أو diagnostic payloads، أو credentials داخل الذاكرة canonical. وتبقى نصوص الجلسات الفرعية مستبعدة من أدلة الترقية.
