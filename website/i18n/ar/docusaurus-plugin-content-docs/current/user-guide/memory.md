...
تسجل كل مهمة حدًا ثابتًا للرسائل مع بيانات تشغيلية محدودة، ولا تخزن نسخة من النص. لذلك لا يشمل التنظيم رسائل لاحقة أضيفت إلى جلسة مستأنفة ويستخدم مساحة عمل الجلسة الأصلية. استخدم `estacoda memory status` أو `estacoda gateway status` لفحص الأعداد الخاصة بالملف الشخصي، واستخدم `estacoda memory finalization list` و`retry` و`prune` لفحص المهام واستردادها محليًا.
