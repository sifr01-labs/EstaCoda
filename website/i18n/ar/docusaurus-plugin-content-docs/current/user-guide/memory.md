...
بدء الجلسة التالية أو الخروج ينتظر فقط كتابة قصيرة ودائمة في قاعدة البيانات، ولا ينتظر استخراج النموذج أو كتابة الذاكرة. تعالج خدمة البوابة المدارة المهمة في الخلفية للملف الشخصي المحدد. إذا كانت الخدمة متوقفة، تبقى المهمة محفوظة في `~/.estacoda/sessions.sqlite` وتعمل عند تشغيل بوابة ذلك الملف الشخصي لاحقًا.

تسجل كل مهمة حدًا ثابتًا للرسائل مع بيانات تشغيلية محدودة، ولا تخزن نسخة من النص. لذلك لا يشمل التنظيم رسائل لاحقة أضيفت إلى جلسة مستأنفة. استخدم `estacoda memory status` أو `estacoda gateway status` لفحص أعداد `pending` و`running` و`retrying` و`failed` الخاصة بالملف الشخصي.
