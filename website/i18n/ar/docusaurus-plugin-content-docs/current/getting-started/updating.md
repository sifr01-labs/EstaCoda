...
| `estacoda update --gateway` | الوضع غير التفاعلي. يُسجّل في `~/.estacoda/logs/update.log`. يُعيد تشغيل خدمة البوابة المُدارة عند النجاح، أو يطبع تعليمة إعادة التشغيل اليدوية. | نفس الافتراضي |
...
- **وضع البوابة لا يخمّن.** يُعيد `--gateway` تشغيل خدمات البوابة المُدارة فقط التي يتم اكتشافها عبر abstraction service-manager. إذا لم يُعثر على خدمة مُدارة، يطبع تعليمة إعادة التشغيل اليدوية. لا يُعيد تشغيل عمليات المستخدم العشوائية أبداً.
...
إعادة تشغيل البوابة تحاول فقط الخدمات المُدارة المُسجلة عبر `estacoda gateway install-service`. إذا كنت تشغل البوابة يدوياً، أعد تشغيلها باستخدام `estacoda gateway restart`.
