...
describe("one-shot prompt", () => {
  ...
  it("prints bounded delegated child lifecycle lines", async () => {
    ...
    expect(result.output).toContain("Subagent 1: started");
    expect(result.output).toContain("Subagent 1: timed out");
    ...
    expect(result.output.split("\n").filter((line) => line.includes("Subagent 1"))).toEqual([
      "Subagent 1: started",
      "Subagent 1: timed out",
    ]);
  });
