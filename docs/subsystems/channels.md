...
Telegram final-answer delivery records a profile-scoped mapping from each delivered chunk ID to its originating visible turn. Replying to any mapped answer chunk with `/usage` inspects that turn directly; a normal-language reply can use the model-visible `session.usage` `replied_turn` scope. Resolution is constrained to the same Telegram account/chat/topic and then re-authorized against the current Session compression lineage. Progress, approval, command, and artifact messages are not attributed as answer turns.
...
Durable delegated Tasks do not keep the creating turn active, so later channel messages follow the ordinary busy-session policy. Control commands still bypass the ordinary queue. `/stop` aborts the foreground turn but does not implicitly cancel a Task whose handle has already been returned. `/status` can show durable Task counts and bounded worker summaries for running Attempts without exposing worker prompts, transcripts, credentials, or tool arguments.
