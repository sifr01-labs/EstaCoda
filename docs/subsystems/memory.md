...
Runtime recall is intentionally narrow. It is only injected for high-confidence continuity language such as "last time", "what did we decide", "what did I say about", "continue from", "we discussed", explicit singular or plural references to previous sessions, or direct references to sites and URLs visited or used in those sessions. Equivalent high-confidence Arabic and mixed-language phrases are supported. Feature requests such as "Create a session history page" and reminders such as "Remember to clear history" do not activate recall.
