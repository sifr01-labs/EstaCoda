...
The managed gateway supervisor claims queued work in the background. First-run setup offers this service even when no channel is configured. A profile lease permits one durable memory mutation per profile across checkpoints, `memory.curate`, automatic promotion, and operator writes. Expired jobs are recoverable, failures use bounded retry before a terminal state, and the worker retains only the latest 1,000 terminal rows per profile. If no gateway service is running, work remains durable until a gateway for that profile runs. Finalization reads only through the captured cutoff and uses the originating session workspace, so later messages and another gateway workspace cannot change its scope.
...
`estacoda memory status` and `estacoda gateway status` report profile-scoped `pending`, `running`, `retrying`, and `failed` finalization counts. Top-level `memory finalization` commands inspect bounded job metadata, requeue failed work, and prune terminal metadata; they are not exposed as remote `/memory` mutations. Checkpoints, `memory.curate`, automatic promotions, and operator writes share the profile curation lease and may report that memory is busy while a background finalizer is writing.
