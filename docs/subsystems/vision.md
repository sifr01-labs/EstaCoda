...
`vision.analyze` accepts one `path`, or `paths` with two to twenty images. Multiple paths select `compare` when `mode` is omitted. The modes are `describe`, `ocr`, `document`, `chart`, `screenshot`, and `compare`; detail is `low`, `standard`, or `high`; output depth is `concise`, `standard`, or `detailed`. A caller cannot combine `path` and `paths`, use more than twenty images, or use a non-comparison mode for multiple images. Sets above ten images are validated first, then processed sequentially in bounded provider batches of at most ten (or one for a custom route whose repeated-image capability is unknown). Activity reports the current image range. After every batch succeeds, a text-only provider pass synthesizes the complete findings without resending images; if synthesis fails, every batch finding is returned instead. A failed image batch never silently reports the remaining images as analyzed.
