...
Model-facing `delegate_task` results are bounded to 8,000 characters in both initial and continuation prompts. Batch rendering reserves a heading for every child, caps unsuccessful child detail at 800 characters, distributes the remaining budget fairly across successful children, and redistributes unused shares from short reports. Truncated child detail carries an explicit total-character marker. Other tools keep the standard smaller prompt-result limits.
