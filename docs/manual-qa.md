...
| Delegated batch | `estacoda` then request a batch of at least three read-only subtasks | `delegate_task` returns one queued durable Task handle with three Steps without holding the turn open. Task/background status—not nested child rows in the creating turn—reports subsequent execution. |
...
| Task inspection | Focus the Task header with `Tab`/arrows and `Enter`, then repeat by clicking it | Both routes open the same whole-Task workspace. It shows factual state, aggregate tokens/cost, Subagents, Steps, dependencies, approvals, blockers, results, and an event trace. It never shows a completion percentage. |
| Trace history and live tail | In a running Task, select an earlier activity square with arrows and with the mouse; allow new events to arrive; select `Return to live` | The selected event remains outlined by event ID and new activity does not move it. `Return to live` resumes following the newest event. Category counters remain all-time totals when the visible trace window overflows. |
| Subagent inspection | Focus and open a Subagent card with keyboard, then repeat with the mouse | Both routes open the same Step-ID-filtered workspace. Breadcrumb/Escape returns Subagent → Task → main session. Current safe activity, retained trace, retries, usage/cost, dependencies, blockers, results, and artifacts remain scoped to that Subagent. |
| Live Task refresh and resize | Keep an earlier Task or Subagent event selected while provider usage settles, another Task appears, and the terminal is resized | Cost/status refresh without closing inspection; existing cards do not reorder; the selected Task, Subagent, and event remain selected by stable ID; the layout reflows to the current terminal dimensions. |
| Task compatibility modes | Inspect the same Task in dark, light, plain/`NO_COLOR`, Arabic, dumb-terminal, CI, and non-TTY modes | Semantic colors use theme tokens in color modes. Plain output has no background paint or animation but preserves labels and structure. Arabic isolates IDs, handles, paths, costs, and counts. Dumb/CI/non-TTY output uses deterministic `task`/`/task` summaries with no mouse or cursor controls. |
...
| Mouse clicks or wheel events remain captured | Run `printf '\033[?1000l\033[?1002l\033[?1003l\033[?1006l\n'`, then `stty sane`. |
