...
| Delegated batch | `estacoda` then request a batch of at least three read-only leaf subtasks | `Delegated work` appears while the parent `delegate_task` runs, including when the assistant emitted prose before the tool call. One bounded row appears per started child, completed rows update in place, normal child tool/provider events do not enter the transcript, and the frame does not flicker or clear on progress. |
| Delegation settlement | Run a batch containing one fast child and one child configured to time out or fail | Child rows show the precise completed, failed, blocked, cancelled, or timed-out outcome. The live region disappears before visible assistant streaming, and `Tools completed` contains one parent `Delegate Task` row with matching counts and no child rows. |
| Delegation fallback | Run the same prompt through one-shot mode and a non-TTY/plain session | Output contains at most a start and settlement line per child. It contains no intermediate child tool/provider lines, prompts, arguments, paths, provider/model identifiers, cancellation reasons, or child session IDs. |
