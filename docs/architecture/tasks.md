...
- `src/tools/task-tools.ts` exposes the bounded, read-only `task.status` tool, including a capped provider-visible list of Result IDs and opaque handles.
- `src/tools/task-result-tools.ts` exposes authorized, paged `task.result.read` access and explicit `task.result.export` Markdown artifact creation.
...
`task.result.read` and `task.result.export` require both the Task and Result IDs. The active session must have a profile-owned `TaskSessionLink` to that Task. Access survives transcript-preserving compaction only when every lineage hop has the same profile, a matching `parentSessionId` and `compactedFromSessionId`, and a parent ended for compression; ordinary parent/child session relationships grant nothing. Missing, cross-profile, and unauthorized records share the same error. Text and JSON are returned in bounded Unicode-character pages; binary artifacts remain durable but are not transported through a text tool. Read metadata includes the Result disposition and an explicit warning for diagnostic output. Pruned and expired Results are unavailable.

`task.status` renders at most 20 authorized Result IDs, handles, kinds, dispositions, byte counts, and primary markers directly in provider-visible content; an omitted count preserves the bound. `task.result.export` is an explicit shared-state mutation used only when a user asks for a document. It reads and integrity-verifies the original UTF-8 Result without model regeneration, writes an exact `.md` copy under the selected profile's channel-media directory with private permissions, and records a document artifact for the current surface. The tool result wraps the delivery artifact in a metadata envelope so provider-visible metadata does not expose its local path.
...
A successful `delegate_task` Result acquires user-facing answer ownership only when it names a non-terminal root Task and its persisted primary Result Step. The parent provider loop stops after the creation tool executes, so it cannot generate a direct substitute while workers remain outstanding. `AgentLoop` returns and persists a deterministic acknowledgement with the Task ID, lifecycle status, execution state, and later-delivery notice; provider-authored substitute text is not forwarded to transcript, trajectory, learning, channel, or voice consumers. Nested child Tasks remain internal to their owning Task. Inspection-only `synthesis: false` batches have no primary Result Step and continue normally.

Research worker Steps may persist a bounded evidence contract with a distinct normalized scope. Admission verifies that the effective child tools can satisfy the requested live-source and repository-evidence modes. Result acceptance then uses successful persisted tool executions as the source of truth: reported live citations must match URLs observed from `web.search`, and repository references must match workspace-relative paths observed from file discovery and reads. A training-only answer, fabricated citation, or missing required tool use settles as `evidence-contract-unsatisfied`; its safe body is diagnostic, never accepted. Synthesis context includes scopes only for accepted Results and lists unavailable scopes in its coverage manifest.
...
The child runner sends progress through the normal runtime event sink while its heartbeat renews the Attempt lease. After the final provider response it checkpoints the bounded `provider-completed` milestone; after successful result capture and validation it checkpoints `result-captured`. Neither checkpoint contains response bodies, tool arguments, prompts, or paths. The Task journal is the sole durable lifecycle authority. Durable cancellation aborts the child; timeouts and provider, approval, security, tool, JSON, and artifact-capture failures return bounded classifications to the scheduler.

Task inspection projects the primary-result lifecycle from those milestones and existing durable timestamps: `providerCompletedAt`, `resultCapturedAt`, `resultRecordedAt`, `attemptSettledAt`, `taskFinalizedAt`, `deliveryStartedAt`, and `parentDeliveredAt`. Result publication, Attempt/Step settlement, and terminal Task settlement remain one scheduler transaction; delivery remains a separate authorized outbox lifecycle. The projection adds observability only and does not introduce a second state machine or stale-running reconciliation path.
...
Interactive CLI delegation creates this local binding for a default single-worker answer and for a batch synthesis answer. Both flow through the same deterministic assistant-message append and display acknowledgement. Explicit `synthesis: false` work remains inspection-only. A terminal Task without an accepted single-worker or synthesis answer appends one deterministic failure receipt and awaits the same display acknowledgement; diagnostic output is never substituted for an answer. External completion delivery renders the equivalent receipt through its existing at-most-once binding.
