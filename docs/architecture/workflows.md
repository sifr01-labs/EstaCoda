...
The selected profile's gateway supervisor remains the persistent background host. It runs even when no channel adapter is configured, so service mode is a general background host for Tasks and cron rather than a cron-only process. Each supervisor tick starts at most one Task pass; overlapping ticks are skipped. The workspace-bound gateway acquires and heartbeats a fenced background Task host lease before reconciliation or dispatch. A live foreground lease excludes that Task; after graceful foreground release or lease expiry, the gateway acquires the next generation and resumes the saved Attempt through its original open worker session. Terminal Tasks release background ownership. Shutdown drain waits for active Task work as well as channel turns, and the shared session database remains open until active Task/finalization work settles.
...
Schema version 19 now backs foreground-first execution and background takeover. The interactive process acquires and heartbeats foreground ownership; the workspace-bound gateway acquires a later background generation only after the foreground lease is released or expires. Graceful shutdown checkpoints and requeues unfinished Attempts immediately, while an ungraceful crash remains recoverable through Attempt and host lease expiry. If no gateway is running, the Task remains durably `waiting_for_host` until an eligible background host starts. Durable foreground approvals and ownership UX remain subsequent integrations.
