...
Process shutdown first closes scheduler admission so no new Steps can be claimed, then waits for every already-dispatched batch to finish its fenced execution and final durable Task-state update. Host-level interruption and ownership handoff build on this boundary; the scheduler does not silently abandon a claimed Attempt halfway through settlement.
...
The selected profile's gateway supervisor remains the persistent background host. It runs even when no channel adapter is configured, so service mode is a general background host for Tasks and cron rather than a cron-only process. Each supervisor tick starts at most one Task pass; overlapping ticks are skipped. A live foreground Task lease excludes that Task from background executor creation, reconciliation, and dispatch. Shutdown drain waits for active Task work as well as channel turns, and the shared session database remains open until active Task/finalization work settles.
...
Schema version 19 now backs foreground-first execution: the interactive process acquires and heartbeats foreground ownership, while the gateway leaves actively leased Tasks untouched. Foreground shutdown currently closes admission, drains already-owned Attempts through durable settlement, releases its Task leases, and disposes its dedicated runtime. Graceful interruption and immediate gateway takeover of unfinished work are the next handoff layer; workspace-specific background executor selection, durable foreground approvals, and ownership UX remain subsequent integrations.
