...
`delegate_task` is a durable Task-creation surface. It validates the request, persists one fixed Task graph, and returns the Task handle immediately. A single request creates one Step. A batch creates independent worker Steps plus one terminal `synthesis` agent Step with immutable dependencies on every worker in revision 1. A synthesis object customizes the final-answer objective/model; `synthesis: false` explicitly creates inspection-only batch work. The Task scheduler—not the provider turn or an in-memory delegation manager—owns concurrency, retries, cancellation, recovery, results, and settlement.

Answer ownership is enforced at the shared provider-loop boundary. After successful `delegate_task` execution, bounded tool-result metadata is accepted only when it names a non-terminal root Task and a `primaryResultStepId`. That receipt stops the provider loop before post-tool continuation, and `AgentLoop` replaces provider-authored final text with a deterministic Task acknowledgement before transcript, trajectory, learning, channel, or voice handling. The same response therefore reaches CLI and gateway surfaces. Nested child Tasks remain internal to their owning Task, while `synthesis: false` carries no primary Result Step and does not acquire ownership. Completion services later deliver the accepted primary Result exactly once; when none exists, they deliver a deterministic no-answer receipt rather than reopening parent generation.

In an interactive CLI process, an `auto` Task belongs first to a dedicated foreground runtime rather than the provider turn or active conversation runtime. Task creation activates that process host only after the graph is durable and waits for initial Attempt dispatch, not worker completion. An explicit `background` preference bypasses foreground admission so the gateway owns it from the beginning. The host retains its own database connection, executor, ownership heartbeat, and scheduler across conversation turns and session switches. On process shutdown it gives active work a short settlement grace. Work that has not started, or whose persisted Step is `idempotent` or `retry_safe`, is durably requeued without consuming an ordinary retry and retains its open worker session. Started `unknown` or `non_idempotent` work is fenced and paused for explicit operator review instead of being executed twice. The host then releases foreground ownership. The gateway resolves the Task's persisted canonical workspace against the live filesystem, rechecks trust, acquires a fenced background lease, and lazily creates an executor bound to that exact workspace. One gateway may own Tasks from several verified workspaces without sharing their executor runtimes; late foreground results cannot overwrite resumed execution.

The synthesis Step has no child-Task authority and can read only bounded accepted worker Result handles through `task.result.read`. It cannot run before all workers settle. When some workers fail, cancel, or skip, synthesis may still run only if at least one completed worker published an accepted Result; its bounded context identifies unavailable coverage and requires the final answer to qualify conclusions explicitly. Diagnostic Results remain inspection-only and never enter synthesis context. If no accepted worker Result exists, synthesis is skipped and the Task settles `partial` without an unsupported answer. Delegated leaf and synthesis Steps are classified `retry_safe` only from this persisted, read-only authority; orchestrators or other Steps with shared-state mutation authority remain `unknown`. Automatic retries are reserved for lease-loss recovery, so ordinary provider or tool failures still follow the graph's failure policy. A completed partial synthesis remains the Task's primary terminal Result for operator projections and completion delivery, while worker Results remain separately readable.

`createRuntime` provides a profile-bound `DurableDelegationService` only when durable SQLite Task persistence is available. The visible turn and stable provider tool-call identity jointly scope the Task creation key, so provider call IDs reused in later turns create distinct Tasks. A replay within the same turn/request returns the existing Task only when the normalized graph definition matches; any conflict—including changing the default batch synthesis choice—fails closed. The visible turn remains the stable root-turn attribution. Foreground activation runs only after commit, and a failure returns the durable handle with a redacted activation marker instead of losing access to the committed Task; explicit operator Task creation follows the same post-commit return contract. Calls made from a running orchestrator Step create linked child Tasks only when that Step's persisted policy is `fire_and_forget`; `forbid` fails closed. New child creation also requires the worker's exact, unexpired Attempt lease generation, with no pending cancellation, so a superseded worker cannot create descendants after losing its fence. An exact replay may still recover the already-created handle because it performs no new creation or budget reservation. Descendants inherit root Task and origin-session/turn attribution, while authority and budget must remain narrower than the active parent Step. The origin session is atomically linked as an observer. Root and child Tasks retain the same canonical workspace binding and recheck live trust before execution. Runtime children never alter the parent's immutable dependency graph.
...
The parent input in step 1 is the final provider-visible inventory after tool availability and disabled-toolset filtering. Explicit tool and toolset lists are also admission requirements: a missing request or an empty effective set prevents Task creation and returns structured diagnostics. A successful Step stores a bounded access audit with the immutable graph, and worker construction revalidates that the runtime set is non-empty and no wider before any provider call. Exact creation replay uses the stored decision rather than recomputing it from a changed live registry.

An optional research contract is also immutable Step authority. Its normalized scope must be distinct within a batch. A live-source requirement is admitted only when `web.search` is effective; a repository-evidence requirement additionally needs `file.read` and one of `file.search`, `file.grep`, or `file.glob`. The worker objective carries the persisted scope and citation rules. Before a successful Result can be captured, `AgentStepExecutor` compares the answer against successful tool-execution records: live HTTP(S) citations must occur in observed search output, and repository references must name workspace-relative paths observed in successful discovery and reads. Failure is classified `evidence-contract-unsatisfied`; safe output remains diagnostic-only and is excluded from synthesis. Accepted dependency references include their research scope, and partial coverage names missing scopes explicitly.

The Task approval policy evaluates hardline command denies first and persists any approval wait against the Task Attempt. Foreground and gateway Task hosts use the same profile-owned SQLite approval queue, bind resolution to the creator session, and replay queue insertion by durable approval-link ID. The interactive session projects only exact creator-session Task approvals into the existing Operator Console approval card and offers approve-once or reject controls; ordinary prompt submission cannot resolve one, and resolution cannot create a broader grant. The foreground host reconciles the queue and schedules the waiting Task immediately after an explicit decision. A foreground exit therefore transfers unresolved approval reconciliation through Task host ownership instead of creating a second prompt or grant. The policy does not inherit a provider turn's pending approvals or treat Task handles as authorization.
...
Browser work has a narrower evidence-aware no-progress budget than the broad provider-turn ceilings. The loop fingerprints trusted, bounded browser results in memory and distinguishes a new structural observation from repeated evidence even when the page itself did not change. A repeated whole-page observation suppresses only that exhausted tool for the next provider step; other inspections and grounded actions remain available. Target resolution failures mean no action was dispatched and allow one different retarget, while dispatched `no-change`, timeout, and unverified actions are tracked separately. Repeated targets, repeated evidence, or multiple ineffective dispatched actions stop deterministically. State-changing actions reset the guard. Persistent diagnostics record only tool names and counts; they do not record page content, tab data, inputs, hashes, or content-derived fingerprints.
...
Continuation prompts reuse the same native selector and builder path when route gates pass. Native history uses a fixed allowance of approximately 12K tokens regardless of the model's total context window, and assistant/tool groups remain atomic. Older unselected native units are repacked into bounded history summaries instead of being injected wholesale. The final continuation instruction remains the last user message.

Foreground provider execution retries one rate-limited route once after a one-second abortable backoff, within the existing absolute deadline and spend-accounting boundary. If that retry also fails, the normal independently configured fallback chain runs. Diagnostics, auxiliary routes, and read-only evaluation paths do not opt into this retry implicitly.

Within one provider turn, a turn-local feedback ledger keeps the newest tool batch available at the existing per-tool result limits. Once a later batch executes, older results become redacted receipts containing status, risk, target summary, and result size rather than raw result bodies. Flat continuation feedback is capped at approximately 12K characters; only the newest batch contributes ephemeral continuation images. Artifact records remain available through the cumulative artifact-reference layer. Tool results already present as selected native `tool` messages are not duplicated in flat feedback.
...
Routed governed cross-system transfers are preflighted independently of optional Plan text. `ExecutionCapabilityPreflight` derives risk classes, reviewed protected and artifact argument paths, grouped browser relay, result redaction, and configured independent verification relationships from the final session-filtered registry. Provider input cannot supply those facts or mark them ready. Preflight invokes no connector tool, resolves no approval, and proves no remote account permission; missing configuration returns a precise blocker before source-side browser work. When multiple reviewed safeguards are absent, that blocker reports every currently detectable gap together instead of requiring repeated retries.

The Plan schema exposed to the provider carries only step IDs, descriptions, and coordination statuses. After each authoritative receipt is indexed, `ExecutionPlanController` may associate it with one active item through a narrow runtime semantic matcher. Successful navigation and matching unverified mutations become bounded `observed` progress. A unique import or credential-environment mutation becomes `verified` and completes only when a later successful verifier carries the trusted link to that mutation. Low-level browser gestures, failed or gated calls, uncertain settlements, unknown receipt IDs, and provider-authored text cannot manufacture runtime progress. Aggregate or multiply matching work remains active and marks the Plan stale for one compact reconciliation instruction in the next normal prompt; synchronization never dispatches an additional provider call. Runtime evidence annotations survive bounded session persistence and compaction only when they still resolve to indexed receipts. A verified completed item is monotonic and cannot be renamed or reactivated by a model merge. This state remains coordination-only and never overrides security policy or the independently derived final execution outcome.

Each tool execution also carries a runtime-only effect classification derived from the registered tool's effective risk class and reviewed capability metadata. Bounded `execution-evidence-recorded` receipts distinguish reads, mutations, and declared verification operations, preserve connector and redacted target context, and link a successful verifier only to the latest compatible successful mutation in the same visible turn. Compatibility uses the declared tool relationship, connector identity, and target identity when both executions provide one. Plans and provider output cannot author these facts, and receipts never persist raw tool inputs, outputs, protected values, or target keys.

Protected-authentication evidence is monotonic. Challenge departure plus an advanced browser identity and authenticated destination can verify MFA even when authenticated controls were already visible behind the protected overlay. Failed and no-change browser actions do not break pending evidence. Only a successful consequential browser-state change, an explicit authentication error, or a signed-out page can invalidate the causal chain; once authentication is completed, only the latter two explicit failure proofs may regress it.

Final execution outcomes preserve consequential work independently of provider synthesis and Mission state. The harness derives `ConfirmedActionReceipt` records only from successful runtime mutation receipts in the current turn; a mutation becomes verified only when a later successful verification receipt carries the runtime-linked `verifiedMutation` relationship. Mission prose, status, completion flags, and evidence IDs do not classify the final result or create verification. Authoritative failed, blocked, and unavailable receipts determine failure or blocking, while interrupted consequential executions that returned no result remain separate uncertain actions. These receipts contain only bounded call, tool, risk, target, outcome, and verification metadata—never tool input, tool output, page contents, collection contents, or credentials. A failed final provider call therefore cannot erase a confirmed mutation, a stale Mission cannot downgrade completed execution, and failed independent verification reports partial completion while preserving the confirmed mutation.

A skipped equivalent connector write can carry an executor-owned `completedReplayOf` call ID. If the original successful write is independently verified in the same visible turn, the skipped duplicate remains blocked evidence but no longer makes the final outcome incomplete by itself; the outcome records `completed_with_recovered_errors`. Provider/tool-result metadata cannot create this link. Missing originals, unrelated verification, ordinary denials, unavailable tools, uncertain writes, open work, and unmet completion requirements are not cleared by it. Readback correlation rejects conflicting observed resource identifiers even when a workspace identifier matches. Successful-write recovery guidance recognizes available reviewed verification relationships without requiring the stronger journal parser needed to settle uncertain writes.

The API-integration playbook also distinguishes resource existence from configuration readiness and live API testing. It asks the agent to inspect effective URLs, variable precedence, and credential-variable bindings while preserving unrelated configuration. This guidance adds no execution gate and does not authorize a token request or other live API call.
...
Foreground supervision is scoped to the active provider turn, not Mission status. The bounded working set is isolated by profile, session, and visible turn. It reuses reviewed result facts plus non-secret identifier-shaped target arguments from successful MCP calls, including schema-valid target fields such as `workspace` and `collection`; failed calls, arbitrary connector prose, credential-like fields, and runtime/session identifiers cannot contribute facts. A mutation invalidates stale reads before refreshing its successful target locators. Browser session leases are acquired and renewed by the foreground provider turn and released when that turn returns or is cancelled, regardless of whether a plan is absent, active, completed, blocked, or stale.

Successful governed browser downloads contribute a separate cumulative artifact receipt. After raw tool feedback is consumed, the prompt retains only the runtime-owned `artifact://` reference, sanitized filename, SHA-256, validated source origin, MIME type, and size needed to identify and relay the artifact. Arbitrary artifact metadata, backing paths, and artifact contents are excluded. Multi-product API integration guidance processes each product through download, destination import, and read-back verification before starting the next product so completed work remains independently resumable.

Generic no-progress control fingerprints runtime tool calls and results in memory. A new successful call/result pair, authoritative mutation, or declared verification resets the counter; repeated calls, repeated results, failed executions, housekeeping plan calls, and output without tool activity do not manufacture progress. While a durable foreground checkpoint is active, its semantic progress revision remains primary. A new successful connector mutation and at most one matching successful declared verification per mutation remain bounded fallback progress when journal updates lag. Useful discovery also counts: new semantic browser snapshot/find/extract evidence, or a fresh read-only MCP call returning new data (including Markdown inventories). Browser comparison ignores identity/ref/observation-clock churn; a same-input snapshot can still reveal newly expanded controls. Connector rereads, cached results, repeated data and explicit clock changes do not reset the counter. Discovery never settles completion or verification requirements; pending connector results are not verification evidence. After tool activity begins, the runtime sends one recovery nudge after three consecutive no-progress iterations and returns a deterministic local receipt after six. Plan transitions cannot reset this guard. The existing browser observation guard remains the stricter browser-specific path, and the general 45-iteration and 100-tool ceilings remain unchanged.

For compatibility, a foreground Mission containing a `user_input_required` item reopens that plan item when the user supplies a non-empty response or explicitly asks to retry. The first such item becomes active when no other item is already active; otherwise it returns to pending. A `plan merge` transition away from `blocked` or `cancelled` clears its obsolete blocker without requiring a separate null field. These are optional planning-state updates only: a Mission blocker cannot suspend provider execution, trigger a special repair retry, grant tool authority, or bypass ordinary security and approval checks. Trusted authentication receipts surface their own runtime user-input blocker independently of Mission state.
...
Foreground turns build one bounded initial provider inventory after normal availability and disabled-toolset resolution. Ordinary conversation receives no tools. Actionable work combines the intent/task policy, the selected skill's required and available optional non-connector toolsets, every registered tool from connectors explicitly named by the user, the active browser toolset, attachment-required read tools, a compact recovery set (`browser.status`, MCP status, and provider status), and `plan` only for genuinely multi-step work. The bundled `api-integration` skill uses a reviewed non-connector profile: the complete browser interaction set needed for authentication, protected input, tabs, governed downloads, and page dialogs; compact recovery tools; and `browser.vision` only as an evidence-gated expansion candidate. All tools from each explicitly named connector remain available, while generic file writes, broad web tools, setup mutation, unrelated connectors, and diagnostic browser extras are excluded unless another independent rule (such as a ready attachment reader) requires them. Generic MCP routing never exposes unrelated connectors, and routing confidence does not control breadth. An open conversational commitment may reuse connector identities and browser continuity observed in its bounded prior request; current-turn negation, cancellation, supersession, session rotation, or unavailable catalog entries retire that continuity. Raw tool arguments, results, browser contents, secrets, arbitrary toolsets, and provider-authored capability names are never persisted as capability context.

A governed-transfer preflight blocker persists one bounded explicit continuation containing only the sanitized original request, browser toolset identity, configured connector identity, and blocker reason codes. Retry acknowledgements such as “retry” and “why not try again” route with that prior request for skill selection and retain the connector plus compact recovery tools. The latest user message remains authoritative, and cancellation or a new explicit request retires this state. Expanded resources, connector results, credentials, and provider-authored capability names are never copied into it.

Natural resume phrases such as “pick up where we left off” remain attached to a non-terminal foreground checkpoint, including a bounded set of common spacing and omitted-letter typing mistakes. This tolerance does not apply to unrelated explicit requests, cancellation, or new slash commands. If a user pastes an explicitly labelled credential into ordinary chat, the agent loop replaces that turn with a value-free protected-input instruction before routing, event emission, persistence, memory, or provider dispatch. The user must enter the value again through the existing no-echo protected-input prompt. Credential-shaped legacy history is sanitized before both packed and provider-native replay; this reduces replay exposure but does not revoke a credential already disclosed to a prior session or external channel.

The runtime may expand that inventory once between ordinary provider iterations when authoritative execution evidence proves that an eligible capability was omitted. The current evidence-gated expansion is `browser.vision` after the browser observation guard reports failed semantic/native target resolution or a dispatched no-change action. Provider prose, Plan text, and resumed text cannot request or invent expansion. Candidates must already come from the selected skill, a user-named connector, the active browser, or the explicit recovery set, and must exist in the final session-filtered catalog. Initial and expanded inventories persist tool names, expansion reason, connector lifecycle/exposure state, and estimated native-schema tokens; they never persist schemas, arguments, results, browser content, or secrets. Visibility still grants no execution authority: execution-time approvals, secret protection, workspace trust, hard blocks, and provider risk filtering are unchanged. Delegated children and durable Task steps keep their existing inventory rules.

Foreground provider execution returns a runtime-owned termination cause (`normal`, provider failure, budget or deadline exhaustion, browser or tool-loop no-progress, required user input, or cancellation). A readable local stop receipt cannot replace that cause with nominal success. Final classification also applies a coarse completion floor—`none`, `read`, `mutation`, or `mutation_with_verification`—derived from the user request and trusted available tool capability metadata. Only authoritative execution receipts settle the floor. This expectation is not a step ledger, cannot expose tools, and grants no execution authority. A new open conversational commitment is projected before classification, so stopped work, read-only progress toward requested mutation, missing configured verification, and open follow-up work cannot be recorded as completed. Assistant metadata, session diagnostics, and trajectory outcomes persist the same final status, termination cause, and completion floor; learning telemetry receives the corresponding success, partial, blocked, failed, or cancelled projection of that status.

Browser supervision does not enter a universal action-only recovery mode. Distinct observations that reveal new candidates, regions, refs, tabs, or document structure remain valid evidence on an unchanged page. Only exhausted whole-state observation schemas are temporarily suppressed; query-specific tools and the rest of the fixed inventory remain available. A target-not-found or ambiguous result permits one bounded retarget with a different grounded ref or locator. Repeating that target, repeating equivalent evidence, or dispatching repeated ineffective actions ends the turn with `browser_no_progress`. Supervision never executes an action itself and does not bypass browser URL policy, target identity and stale-document checks, structural preflight, protected-input handling, approvals, or execution budgets. Snapshots associate a compact visible text region with its actionable descendants, and a failed exact `browser.find` may return up to four current-document candidates ranked by direct and same-region relevance. Those alternatives are explicitly non-exact and remain subject to every normal action check.

Execution plans are optional coordination state. The runtime does not classify requests into mandatory planning, automatically create provisional execute/verify Missions, or create a Mission from authentication effects. A provider may call `plan` when persistent coordination helps, including in the same batch as substantive tools, but ordinary work starts and completes with no plan at all. Standard tool-result feedback drives continuation. Browser observation, foreground tool-loop progress, runtime authentication blockers, unavailable-tool receipts, approvals, trust, and execution budgets supervise the work directly. Plan creation or status grants no authority and cannot become a prerequisite for tool execution, final outcomes, browser leases, or continuation.
