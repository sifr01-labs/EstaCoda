...
High-confidence routed foreground turns narrow one fixed provider-visible tool inventory after normal availability and disabled-toolset resolution. It contains core and optional `plan` tools, intent-routed and selected-skill toolsets, available optional skill toolsets, attachment-required tools, and every configured connector explicitly named in the current user request. Low-confidence routes retain the wider resolved inventory so a weak route cannot make recovery impossible. Provider-authored or resumed Mission prose, evidence, and capability requirements cannot add tools during the turn. The fixed inventory remains bounded by the final session-filtered catalog and cannot reintroduce unavailable tools, disabled toolsets, per-turn exclusions, blocked provider risk classes, or unavailable MCP capabilities. Execution-time security and approval gates remain unchanged. Delegated children and durable Task steps keep their existing inventories.
