...
The synthesis Step has no child-Task authority and can read only the bounded worker Result handles through `task.result.read`. It cannot run before all workers complete. Delegated leaf and synthesis Steps are classified `retry_safe` only from this persisted, read-only authority; orchestrators or other Steps with shared-state mutation authority remain `unknown`. Automatic retries are reserved for lease-loss recovery, so ordinary provider or tool failures still follow the graph's failure policy. A failed worker blocks and skips synthesis, producing a `partial` Task instead of an unsupported answer. On success, the synthesis Result is the Task's primary terminal Result for operator projections and completion delivery, while worker Results remain separately readable.
