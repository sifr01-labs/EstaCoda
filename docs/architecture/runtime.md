...
Cross-system Missions may declare bounded semantic capability requirements using exact tool names from the current session. `ExecutionCapabilityPreflight` derives risk classes, reviewed protected-argument paths, grouped delivery, browser relay, and configured independent verification relationships from registered tools, then assesses them against the final session-filtered registry and runtime availability. Assessment results are runtime-owned; provider input cannot supply security paths, override risk, or mark a requirement ready. Preflight does not execute a probe or mutation, resolve approval, or prove remote account permissions. A failed requirement may block the optional plan item, but it does not stop or authorize runtime work. Actual unavailable tool calls produce runtime-owned unavailable receipts through registry resolution, while safe remote permission probes still execute through their normal tool and security paths.

Plan completion remains evidence-gated for compatibility. A session-local harness index accepts only successful, allowed `ToolExecutionRecord` values and derives bounded receipts containing the call ID, tool name, risk class, and redacted target. Provider-supplied evidence details, failed or gated calls, `plan`, and `delegate_task` cannot prove completion. When a completion merge omits evidence, the plan tool returns at most eight safe successful candidates from the current visible turn, preferring tools required by the affected item; it never includes raw inputs, outputs, protected values, or earlier-turn calls, and it never chooses evidence for the provider. Reasoning-only completion must be explicit and is rejected for action-shaped items. A merge cannot complete the final unfinished existing objective item while appending new unfinished work; optional follow-up cleanup belongs in the final response or a separate user-authorized request. Required discoveries remain appendable while meaningful original work is still unfinished. An unfinished plan no longer forces provider continuation or replaces a truthful provider result with a Mission-incomplete receipt; standard continuation follows executed tool results and recoverable runtime feedback.
...
High-confidence routed foreground turns narrow one fixed provider-visible tool inventory after normal availability and disabled-toolset resolution. It contains core and optional `plan` tools, intent-routed and selected-skill toolsets, available optional skill toolsets, attachment-required tools, and every configured connector explicitly named in the current user request. An active browser also preserves the browser toolset for deictic actions such as “click the app shown there.” An open conversational commitment may reuse connectors explicitly named in its bounded user request plus the bounded browser toolset and registered connector identities observed during successful execution; current-turn connector negation, cancellation, supersession, session rotation, or unavailable catalog entries retire that continuity. Raw tool arguments, results, browser contents, secrets, arbitrary toolsets, and provider-authored capability names are never persisted as capability context. Low-confidence routes retain the wider resolved inventory only when no connector or continuity signal is available, so a weak route cannot make recovery impossible. Provider-authored or resumed Plan prose, evidence, and capability requirements cannot add tools during the turn. The fixed inventory remains bounded by the final session-filtered catalog and cannot reintroduce unavailable tools, disabled toolsets, per-turn exclusions, blocked provider risk classes, or unavailable MCP capabilities. Tool visibility grants no execution authority; execution-time security and approval gates remain unchanged. Delegated children and durable Task steps keep their existing inventories.
