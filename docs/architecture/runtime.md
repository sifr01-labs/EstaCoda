...
A foreground Mission blocked specifically on `user_input_required` reopens its user-input work when the user supplies a non-empty response or explicitly asks to retry. The first such item becomes active and any later such items return to pending; stale blockers and prior failure evidence are cleared before provider execution so they cannot immediately terminate the resumed turn. Approval, capability, external-state, and budget blockers remain blocked, and reopening planning state never grants tool authority or bypasses ordinary security and approval checks.
