...
Root foreground turns automatically activate a Mission for clearly multi-step actionable requests. The conservative classifier requires an execution or mutation signal plus evidence such as multiple actions, systems, or targets, an explicit sequence, or verification after the action. An explicit request to sign the user in or complete authentication also activates a Mission because credential entry, a possible challenge, and post-submit verification are distinct execution phases. Opening a login page, asking for sign-in advice, simple one-step work, and multi-part read-only questions remain plan-free. When the request text makes activation unambiguous, the first provider request exposes only `plan`. If activation becomes clear only from a proposed multi-system tool batch, the harness holds that batch and retries with only `plan`. In either path the model is instructed to write the Mission before substantive tool execution. If it still omits or misuses `plan`, the harness creates a bounded two-item provisional Mission and retries without executing the rejected batch. This interception grants no authority: the eventual substantive tools still pass through ordinary availability, trust, security, approval, and budget enforcement. Child sessions, delegated agents, and durable Task workers have no foreground plan controller and are unaffected.
