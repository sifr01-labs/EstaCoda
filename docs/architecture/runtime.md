...
Foreground supervision is scoped to the active provider turn, not Mission status. The bounded working set is isolated by profile, session, and visible turn; it reuses confirmed safe identifiers within that turn and invalidates relevant reads after mutation. Browser session leases are acquired and renewed by the foreground provider turn and released when that turn returns or is cancelled, regardless of whether a plan is absent, active, completed, blocked, or stale.
...
Foreground turns build one bounded initial provider inventory after normal availability and disabled-toolset resolution. Ordinary conversation receives no tools. Actionable work combines the intent/task policy, the selected skill's required and available optional non-connector toolsets, every registered tool from connectors explicitly named by the user, the active browser toolset, attachment-required read tools, a compact recovery set (`browser.status`, MCP status, and provider status), and `plan` only for genuinely multi-step work. Generic MCP routing never exposes unrelated connectors, and routing confidence does not control breadth. An open conversational commitment may reuse connector identities and browser continuity observed in its bounded prior request; current-turn negation, cancellation, supersession, session rotation, or unavailable catalog entries retire that continuity. Raw tool arguments, results, browser contents, secrets, arbitrary toolsets, and provider-authored capability names are never persisted as capability context.
