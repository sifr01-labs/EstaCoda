...
Cross-system Missions may declare bounded capability requirements using exact tool names from the current session. `ExecutionCapabilityPreflight` assesses those declarations against the final session-filtered registry, tool availability, risk classes, reviewed protected-argument paths, protected-transfer availability, and a separate read-safe verification tool. Assessment results are runtime-owned; provider input cannot mark a requirement ready. Preflight does not execute a probe or mutation, resolve approval, or prove remote account permissions. A failed mandatory requirement blocks its associated item and stops the turn after `plan` before source-side exploration begins. Any safe remote permission probe still executes later through its normal tool and security path.
