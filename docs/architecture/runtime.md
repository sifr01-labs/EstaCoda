...
Successful browser observations have a narrower no-progress budget than the broad provider-turn ceilings. The loop compares consecutive `browser.snapshot` and `browser.tabs` state in memory, sends one local-only recovery nudge after the second unchanged observation, and stops deterministically after the third. A changed observation or another tool outcome resets the guard. Persistent budget diagnostics record only the tool name and counts; they do not record page content, tab data, hashes, or content-derived fingerprints.
...
Repeated unchanged browser observations first receive one corrective nudge and then enter a bounded action-recovery window instead of stopping immediately. During recovery, redundant observation schemas are suppressed while existing browser action schemas remain available; `browser.tabs` may appear once only when the runtime lacks a current complete tab inventory. A successful state-changing action restores the normal turn inventory. An unchanged or failed action, an unavailable grounded action, or a provider response that declines to act ends the window with `browser_no_progress`. Recovery never executes an action itself and does not bypass browser URL policy, target identity and stale-document checks, preflight, protected-input handling, approvals, or execution budgets. A failed exact `browser.find` may return up to four structurally grounded current-document candidates when their bounded text overlaps the locator; those refs are labeled non-exact and remain subject to the same action checks.
