...
A foreground Mission containing a `user_input_required` item reopens that work when the user supplies a non-empty response or explicitly asks to retry, even when other pending work keeps the Mission's aggregate status active. The first such item becomes active when no other item is already active; otherwise it returns to pending. Stale blockers and prior failure evidence are cleared before provider execution so they cannot immediately terminate the resumed turn. A `plan merge` transition away from `blocked` or `cancelled` likewise clears the obsolete blocker without requiring a separate null field. If that plan update is invalid, the provider receives one bounded repair continuation before the existing blocker is surfaced again. Approval, capability, external-state, and budget blockers remain blocked, and reopening planning state never grants tool authority or bypasses ordinary security and approval checks.
...
High-confidence routed foreground turns narrow the initial provider-visible tool schemas after normal availability and disabled-toolset resolution. The initial inventory contains core and `plan` tools, routed and selected-skill toolsets, available optional skill toolsets, attachment-required tools, and canonical tools referenced by a resumed Mission. During the same foreground turn, an accepted Mission may add exact capability requirements whose runtime-owned preflight assessment is `ready`; additions are monotonic and take effect on the next provider iteration. Low-confidence routes retain the wider resolved inventory so a weak route cannot make recovery impossible. Both initial narrowing and Mission expansion remain bounded by the final session-filtered catalog: they cannot reintroduce unavailable tools, disabled toolsets, per-turn exclusions, blocked provider risk classes, or an unavailable MCP capability, and they do not change execution-time security or approval gates. Delegated children and durable Task steps keep their existing inventories and never gain `plan` or Mission-driven expansion through this path.

Root foreground turns automatically activate a Mission for clearly multi-step actionable requests. The conservative classifier requires an execution or mutation signal plus evidence such as multiple actions, systems, or targets, an explicit sequence, or verification after the action. An explicit request to sign the user in or complete authentication also activates a Mission because credential entry, a possible challenge, and post-submit verification are distinct execution phases. Opening a login page, asking for sign-in advice, simple one-step work, and multi-part read-only questions remain plan-free. When request text makes activation unambiguous, the runtime creates a bounded two-item provisional Mission before the first provider request. That request retains the ordinarily narrowed tool inventory; activation does not reduce it to `plan` or delay a substantive tool batch. If activation becomes clear only from a proposed multi-system batch, the runtime creates the provisional Mission before executing the accepted batch without discarding it. A provider `plan` call may refine the provisional state in the same batch. Mission activation grants no authority: substantive tools still pass through ordinary availability, trust, security, approval, and budget enforcement. Child sessions, delegated agents, and durable Task workers have no foreground plan controller and are unaffected.
