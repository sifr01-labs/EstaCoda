...
| Provider-call idempotency | A stable provider tool-call ID derives the Task creation key. Exact replay of the normalized graph definition returns the existing handle; changing explicit or default synthesis semantics or any other graph input fails closed. | `src/delegation/durable-delegation-service.test.ts`, `src/tools/delegation-tools.test.ts` |
...
| Root/child ownership | Root Tasks retain creator-session ownership. Nested calls persist `parentTaskId`, `parentAttemptId`, and an agent actor bound to the active Attempt. New children require the exact current, unexpired parent lease generation and no pending cancellation; stale workers may only recover an exact existing handle. | `src/delegation/durable-delegation-service.test.ts`, `src/workflow/agent-step-executor.test.ts`, `src/workflow/sqlite-task-store.test.ts` |
| Child lifecycle semantics | Every Step persists `forbid` or `fire_and_forget`. Worker Steps forbid runtime children; authorized orchestrators may create only detached children. Parent dependencies are never mutated and parent completion does not wait for detached children. | `src/contracts/task.test.ts`, `src/delegation/durable-delegation-service.test.ts`, `src/workflow/sqlite-task-store.test.ts` |
