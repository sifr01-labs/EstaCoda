...
Install Python-side dependencies in the operator environment that runs EstaCoda. The Node package does not install Python faster-whisper for you.
...
Then make sure the gateway or CLI process can find that Python environment. The worker file is packaged at:
...
- Set `stt.local.fasterWhisper.allowModelDownload: true` for non-gateway first-run download allowance.
- Use `hfHome` to keep model cache under an operator-chosen path.
- Without `hfHome`, existing `HF_HOME` or `TRANSFORMERS_CACHE` is respected where the worker environment provides it.
...
| `python package missing` | faster-whisper import fails. | Install Python dependencies in the runtime environment. |
...
| `hfHome` or Hugging Face cache env | faster-whisper model cache. |
...
- Enable faster-whisper downloads only when model-fetch side effects are acceptable.
- Keep ffmpeg and Python dependencies pinned or managed by the operator environment.
