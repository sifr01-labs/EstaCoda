...
`list` defaults to 20 entries and accepts a limit from 1 to 100. `show` reports lifecycle status separately from live execution ownership (`foreground`, `background`, or `waiting`), the persisted preference, foreground-owner activity, background-continuation readiness, a bounded waiting reason, Step progress, usage, results, trust, and host state. It attributes immutable provider usage to each Attempt and rolls those charges through its Step or worker into the complete descendant Task tree, including failed Attempts, retries, fallbacks, and synthesis. When a Task spending limit is configured, `show` also reports spent, reserved, remaining, and limit amounts from the canonical budget scope. Terminal Tasks display execution as settled. `result` lists bounded opaque result handles and summaries; it does not print result bodies.

Cost presentation distinguishes four states: `$0.00` is a recorded zero, `$0.42` is a complete recorded estimate, `at least $0.42` is a known lower bound when some provider pricing is unavailable, and `unavailable` means no trustworthy monetary estimate can be shown. Expanded views explain partial pricing on a separate line; compact rails use the equivalent `≥` marker. Existing sessions are not backfilled or classified specially.
...
In a supported interactive TTY, linked Tasks also appear as retained cards in the Operator Console, including after settlement. Use `Ctrl+T` or an available `Tab` transition to focus the cards, arrow keys to select, and `Enter` to inspect. On a focused Task, `T` enters its logical activity trace; `Left`/`Right` and `Home`/`End` select history or return live, `Enter` inspects the selection, and `Escape` collapses the trace. State-valid controls expose `R` retry, `D` detach, `P` pause, and confirmed `C` cancel; invalid mutation controls are omitted, and settled Tasks keep trace inspection only. The modal view supports arrow scrolling, `Page Up`/`Page Down`, `Home`/`End`, and `Escape` to return. It shows the active plan, Step dependencies and Attempts, elapsed time, safe activity labels, coarse tool category, per-Step and per-Attempt spending, Task budget state, result handles, and bounded wait/failure information. Plain and non-TTY users use `task list`, `task show`, and `task result` for the same durable state.

CLI completion delivery persists a bounded final logical-trace snapshot with the session message and renders its compact ribbon above the accepted answer in both structured Operator Console and plain output. The ribbon uses the actual output width. New snapshots retain the total worker count, while existing completion messages without a snapshot and version-1 snapshots without that total remain compatible. The snapshot contains safe activity categories, scopes, durations, terminal outcome, and bounded worker counts only; it does not duplicate raw Events, worker transcripts, tool payloads, or Result bodies.
