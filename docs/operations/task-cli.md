...
CLI completion delivery persists a bounded final logical-trace snapshot with the session message and renders its compact ribbon above the accepted answer. Existing completion messages without the snapshot remain compatible. The snapshot contains safe activity categories, scopes, durations, terminal outcome, and bounded worker counts only; it does not duplicate raw Events, worker transcripts, tool payloads, or Result bodies.
